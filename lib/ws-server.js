...
WSServer.prototype.onNewConnection = function(ws) {
  ...
  if (ws.upgradeReq.headers['sec-websocket-key'] == null) {
    LOG.E(new CHError('NO_VALID_CLIENT_KEY'));
    return ws.terminate();
  }
...
WSServer.prototype.onServerError = function(error) {
  LOG.E(new CHError('WEBSOCKET_SERVER_ERROR', error.message));
};
  ...
  this.wsServer.findAllEventSubsribers(this, clientKey, function(err, subscribers) {
    ...
    for (var s in subscribers) {
      ...
      this.wsServer.eventUnsubscribe(subscriber, opt.deviceID, opt.serviceID, opt.device_access_token, function(err) {
        if (err) {
          LOG.E(new CHError('WEBSOCKET_UNSUBSCRIPTION_ERROR', err.message));
        }
        this.wsServer.removeEventSubscriber(subscriber, function(error, subscriber, options) {
          if (error) {
            LOG.E(new CHError('WEBSOCKET_REMOVE_SUBSCRIBER_ERROR', error.message));
          }
        }.bind(this));
      }.bind(this));
    }
...
WSServer.prototype.onInputMessage = function(message, flags) {
  ...
  try {
    ...
    return this.send(JSON.stringify({topic: 'countinghouse error', message: 'socket input message not in JSON format'}));
  }
  ...
  switch(inputMessage.topic) {
    case 'subscribe':
      ...
      if (options == null) {
        return this.send(JSON.stringify({topic: 'countinghouse error', message: 'socket subscription options invalid'}));
      }
      ...
      if (deviceID == null || serviceID == null) {
        return this.send(JSON.stringify({topic: 'countinghouse error', message: 'unknown socket subscription options'}));
      }
    ...
    case 'unsubscribe':
      var options = inputMessage.options;
      if (options == null) {
        return this.send(JSON.stringify({topic: 'countinghouse error', message: 'socket unsubscription options invalid'}));
      }
      ...
      if (deviceID == null || serviceID == null) {
        return this.send(JSON.stringify({topic: 'countinghouse error', message: 'unknown socket unsubscription options'}));
      }
      ...
      this.wsServer.findEventSubscriber(this, clientKey, options, function(subscriber) {
        if (subscriber === null) {
          return this.send(JSON.stringify({topic: 'countinghouse error', message: 'cannot find existing event subscription'}));
        }
        var opt = subscriber.options;
        this.wsServer.eventUnsubscribe(subscriber, opt.deviceID, opt.serviceID, opt.device_access_token, function(err) {
          if (err) {
            return this.send(JSON.stringify({topic: 'countinghouse error', message: err.message}));
          }
          this.wsServer.removeEventSubscriber(subscriber, function(error, subscriber, options) {
            if (error) {
              return this.send(JSON.stringify({topic: 'countinghouse error', message: error.message}));
            }
          }.bind(this));
        }.bind(this));
      }.bind(this));
      break;
    default:
      return this.send(JSON.stringify({topic: 'countinghouse error', message: 'unknown socket input message'}));
      break;
  }
};

WSServer.prototype.onSocketError = function(error) {
  LOG.E(new CHError('WEBSOCKET_ERROR', error.message));
};
