...
var validateUser = function(req, res, next) {
  if (options.isDebug === true) {
    var session = new Session(req, res, 'debug', 'debug', 0);
    req.session = session;
    return next();
  }
  ...
  usersDB.view('user', 'byAppKey', { key: appkey }, function(err, doc) {
    ...
    if (doc.rows.length > 0) {
      ...
      if (hasDevice === false) {
        return res.status(500).json({topic: new CdifError().topic, message: 'device not in user\'s device list'});
      }
      ...
      //TODO: build user's freeAPICount obj and send to session, if any freeAPICount then do this, if not then leave it null
