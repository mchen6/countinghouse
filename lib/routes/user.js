...
var validateUser = function(req, res, next) {
  if (options.isDebug === true) {
    var session = new Session(req, res, 'debug', 'debug', 0, req.params.deviceID, null, null);
    req.session = session;
    return next();
  }
    ...
    if (results[0] == null) { // key not exists
      usersDB.view('user', 'byAppKeyWithBasicInfo', { key: appKey }, function(err, doc) {
        ...
        if (doc.rows.length > 0) {
          ...
          var session = new Session(req, res, userName, appKey, balance, deviceID, apiRemainCount, null);
