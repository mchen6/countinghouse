...
module.exports = function(mm, cdifInterface) {
  ...
  router.route('/').get(function(req, res) {
    ...
    var appkey = req.get('X-MCPForge-Key');

    if (appkey == null) {
      return res.status(500).json({topic: 'mcpforge error', message: '用户appKey非法'});
    }

    if (usersDB == null) {
      return res.status(500).json({topic: 'mcpforge error', message: '用户数据库不可用'});
    }

    if (devicesDB == null) {
      return res.status(500).json({topic: 'mcpforge error', message: '设备数据库不可用'});
    }

    usersDB.view('user', 'byAppKey', { key: appkey }, function(err, doc) {
      if (err) {
        return res.status(500).json({topic: 'mcpforge error', message: err.message});
      }
      if (doc.rows.length > 0) {
        ...
        res.status(500).json({topic: 'mcpforge error', message: '未知用户: ' + appkey});
      }
    });
