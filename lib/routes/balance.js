...
module.exports = function(mm, cdifInterface) {
  ...
  router.route('/').get((req, res) => {
    ...
    doUserAuth(req, res, null, appKey, null, null, null, (err, session) => {
      ...
      // Rate limited (7.0.0). Authentication alone left this pollable as fast
      // as an authenticated caller liked, one Redis round trip per request --
      // recorded in docs/cross-cutting-matrix.md as "none. Recorded, not
      // fixed" until now. Gated on the *resolved* session.appKey, not the
      // header this route read above, for the same reason the balance itself
      // is: a caller must not be able to spend another key's budget by
      // quoting it.
