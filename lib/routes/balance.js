...
// -- the same use of doUserAuth lib/routes/admin-only.js and
// lib/socket-server.js's handshake already make. Balance is deliberately
...
module.exports = function(mm, cdifInterface) {
  ...
  router.route('/').get(function(req, res) {
    ...
    doUserAuth(req, res, null, appKey, null, null, null, function(err, session) {
      ...
      cdifInterface.checkBalance(session.appKey, function(err, result) {
        if (err != null) {
