  ...
  router.route('/').post(function(req, res) {
    ...
    if (req.headers['content-type'] === 'application/bson') {
      try {
        ...
        return session.callbackWithoutTimer(new McpForgeError('INVALID_BSON_REQUEST', e.message));
      }
    }
    ...
    if (serviceID == null)  return session.callbackWithoutTimer(new McpForgeError('SERVICEID_NOT_AVAILABLE'));
    if (actionName == null) return session.callbackWithoutTimer(new McpForgeError('ACTIONNAME_NOT_AVAILABLE'));
