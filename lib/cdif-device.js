...
CdifDevice.prototype.subscribeDeviceEvent = function(subscriber, serviceID, actionName, input, inputKey, callback) {
  var service = this.services[serviceID];
  if (service == null) {
    return callback(new DeviceError('SERVICE_NOT_FOUND', serviceID));
  }

  service.subscribeEvent(subscriber, actionName, input, inputKey, callback);
  // , function(err) {
  //   if (!err) {
...
CdifDevice.prototype.unsubscribeDeviceEvent = function(subscriber, serviceID, actionName, input, inputKey, callback) {
  var service = this.services[serviceID];
  if (service == null) {
    return callback(new DeviceError('SERVICE_NOT_FOUND', serviceID));
  }

  service.unsubscribeEvent(subscriber, actionName, input, inputKey, callback);
