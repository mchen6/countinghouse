...
CdifDevice.prototype.setAction = function(serviceID, actionName, action) {
  ...
  return LOG.DE(this, new DeviceError('cannot set action: ' + serviceID + ' ' + actionName));
};
...
CdifDevice.prototype.getDeviceSpec = function(session) {
  if (this.spec === null) {
    return session.callback(new CdifError('cannot get device spec'), null);
  }
...
CdifDevice.prototype.getServiceStates = function(serviceID, session) {
  ...
  var service = this.services[serviceID];
  if (service == null) {
    return callback(new DeviceError('service not found: ' + serviceID), null);
  }
...
CdifDevice.prototype.subscribeDeviceEvent = function(subscriber, serviceID, session) {
  var service = this.services[serviceID];
  if (service == null) {
    return session.callback(new DeviceError('cannot subscribe to unknown serviceID: ' + serviceID));
  }
...
CdifDevice.prototype.unSubscribeDeviceEvent = function(subscriber, serviceID, session) {
  var service = this.services[serviceID];
  if (service == null) {
    return session.callback(new CdifError('cannot unsubscribe from unknown serviceID: ' + serviceID));
  }
...
CdifDevice.prototype.getDeviceRootUrl = function(session) {
  if (this.spec.device.devicePresentation !== true || typeof(this._getDeviceRootUrl) !== 'function') {
    return session.callback(new DeviceError('this device do not support presentation'), null);
  }
  this._getDeviceRootUrl(function(err, data) {
    if (err) {
      return session.callback(new DeviceError('get device root url failed: ' + err.message), null);
    }
    try {
      url.parse(data);
    } catch(e) {
      return session.callback(new DeviceError('device root url parse failed: ' + e.message), null);
    }
...
CdifDevice.prototype.setOAuthAccessToken = function(params, session) {
  if (typeof(this._setOAuthAccessToken) === 'function') {
    ...
    session.callback(new CdifError('cannot set device oauth access token: no available device interface'));
  }
};
