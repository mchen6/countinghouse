...
DeviceManager.prototype.onEventSubscribe = function(subscriber, deviceID, serviceID, actionName, inputKey, token, callback) {
  this.ensureDeviceState(deviceID, token, function(err, cdifDevice) {
    if (err) return callback(err);
    cdifDevice.subscribeDeviceEvent(subscriber, serviceID, actionName, inputKey, callback);
  });
};
