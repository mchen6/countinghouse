  ...
  if (isMainThread === true) {
    for (const deviceID in this.deviceMap) {
      ...
      this.verifyComposition(deviceID, spec, packageInfo, modulePath, (err, composition) => {
        if (err != null) {
          // A broken chain must not serve traffic: log (this is the
          // primary way an author learns about it -- see verifyComposition's
          // own comment) and purge just this deviceID, not the whole
...
// resolved once, here, after discovery has finished. A typo becomes a
// startup error naming the module and the address, instead of a runtime
// failure the first time that code path is reached -- and since
...
// exact file to fix.
//
// Called from onAllModulesDiscovered, once per registered device, after
// allDevicesLoaded is set and the notifyDeviceLoad drain has run: resolution
// below goes through the same 'querydevice' path that queues while
// discovery is in flight, so a composing module may declare a target that
// loads after it.
//
// callback(err, composition): err non-null means the module must not come
// online. On success, composition is {identity, allowed} -- exactly the
// shape handlerCtx.setComposition expects -- or undefined when the module
