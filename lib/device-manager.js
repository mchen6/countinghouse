...
var PeerChannelBroker = require('./peer-channel-broker');
...
DeviceManager.prototype.invokeActionViaPeerChannel = function(appKey, deviceID, serviceID, actionName, args, callback) {
  var workerId = this.peerDeviceWorkerMap[deviceID];
  var channel  = (workerId != null) ? this.peerChannels[workerId] : null;
  ...
  if (channel != null && channel.closed !== true) {
    ...
    return channel.invoke(deviceID, serviceID, actionName, args.input, options.requestTimeout, function(err, data) {
  ...
  this.pendingPeerInvokes[deviceID].push({serviceID: serviceID, actionName: actionName, args: args, callback: callback});
};
...
DeviceManager.prototype.onPeerChannelGrant = function(msg) {
  ...
  var targetWorkerId = msg.targetWorkerId;
...
DeviceManager.prototype.onPeerChannelDeny = function(msg) {
  var err = new DeviceError(msg.errCode != null ? msg.errCode : 'PEER_CHANNEL_DENIED', msg.errMsg);
...
DeviceManager.prototype.flushPendingPeerInvokes = function(deviceID, workerId, err) {
  var queued = this.pendingPeerInvokes[deviceID];
  ...
  queued.forEach(function(item) {
    ...
    channel.invoke(deviceID, item.serviceID, item.actionName, item.args.input, options.requestTimeout, function(cErr, data) {
      if (cErr != null) return item.callback(new DeviceError(cErr.code != null ? cErr.code : cErr.message), data);
      return item.callback(null, data);
    });
  });
};
...
// incoming peer-invoke on this channel gets timed and reported back to the
// main thread's PeerChannelBroker.handleMetering after execution.
DeviceManager.prototype.onPeerChannelOpen = function(msg) {
  var _this = this;
  var callerWorkerId = msg.callerWorkerId;
  var appKey         = msg.appKey;

  var channel = new PeerChannel(msg.port, {
    ...
    onInvoke: function(request, reply) {
      ...
      CHUtil.ci.invokeDeviceAction(request.deviceID, request.serviceID, request.actionName, {input: request.input}, null, function(err, data) {
        _this.workerMessage.sendPeerMeteringToParent({
          callerModule: appKey,
...
DeviceManager.prototype.onPeerInvalidate = function(msg) {
  var channel = this.peerChannels[msg.workerId];
...
DeviceManager.prototype.onWorkerLoaded = function(workerMessage) {
  ...
  workerMessage.on('devicelog', function(message, wm) {
    ...
    CHUtil.__deviceLogWithID(deviceID, data, timestamp, function(e) {
  ...
  if (options.workerThread === true && isMainThread === true) {
    for (var deviceID in this.deviceMap) {
      if (this.deviceMap[deviceID] === moduleInstance) {
        ...
        CHUtil.__deviceLogWithID(deviceID, reason, Date.now(), function(){});
    ...
    // D4: this fires on every path that can make moduleInstance's device
    // state stale to other workers -- module reload (ModuleManager.
    // onModuleLoad's "module reloaded" branch), external unload
    // (onModuleUnload), and worker crash (module-manager.js's worker
    // 'error'/'exit' handlers, which both route through unloadModule ->
    // onModuleUnload) all emit 'purgedevice', so this one hook covers all
    // three of D4's triggers without needing to be duplicated at each of
    // those call sites.
...
DeviceManager.prototype.getAllDeviceSpecs = function() {
  ...
  for (var i in this.deviceMap) {
    ...
    if (cdifDevice instanceof WorkerMessage) {
      for (var deviceID in cdifDevice.deviceList) {
