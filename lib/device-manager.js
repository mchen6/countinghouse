...
DeviceManager.prototype.verifyComposition = function(deviceID, spec, packageInfo, modulePath, callback) {
  ...
  // module that declared "calls", which was never the intent -- see the
  // packageName fallback below the identity lookup for what that check
  // was actually trying to catch (an author binding the wrong name in
  // auth config).
  //
  ...
  const noIdentityBoundError = () => new CHError('MODULE_COMPOSITION_INVALID', friendlyName,
  ...
  authProvider.identityForModule(friendlyName, (identErr, identity) => {
    ...
    if (identity.apiKey == null) {
      // Nothing is bound to this device's own friendlyName. Before giving
      // up: if package.json's name differs (a multi-device module, or an
      // author who assumed composition keys off the package name), check
      // whether THAT name has a binding instead -- if so, the identity
      // exists, it is just filed under the wrong name, which is a much
      // more specific, fixable diagnostic than "nothing bound at all".
      if (packageName != null && packageName !== friendlyName) {
        return authProvider.identityForModule(packageName, (pkgErr, pkgIdentity) => {
          if (pkgErr == null && pkgIdentity != null && pkgIdentity.apiKey != null) {
            return callback(new CHError('MODULE_COMPOSITION_INVALID', friendlyName,
              `${options.authConfigPath} binds an identity to "${packageName}" (package.json's ` +
              `"name"), but composition binds by this device's own friendlyName, "${friendlyName}" ` +
              `-- move it to "runsModules" for "${friendlyName}" in ${options.authConfigPath}.`));
          }
          return callback(noIdentityBoundError());
        });
      }
      return callback(noIdentityBoundError());
    }
