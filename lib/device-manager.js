  ...
  if (isMainThread === true) {
    ...
    for (const deviceID in this.deviceMap) {
      ...
      this.verifyComposition(deviceID, spec, packageInfo, modulePath, (err, composition) => {
        ...
        // No "countinghouse.calls" declared: verifyComposition's documented
        ...
        if (fromWorker) {
          ...
          cdifDevice.sendSetCompositionMessage(
            {deviceID: deviceID, identity: composition.identity, allowed: composition.allowed},
            (relayErr) => {
              if (relayErr != null) {
                LOG.E(new CHError('MODULE_COMPOSITION_INVALID', deviceID,
          ...
          handlerCtx.setComposition(cdifDevice, composition);
        }
      });
    }
  }
...
// and since
// ctx.call's own rejection never reaches an MCP client with any detail
// (lib/mcp/gateway.js's toolCallResult discards the fault payload, and the
// worker hop re-wraps the error from its code alone), this function's error
// messages are the real, and only, user-facing surface for a misconfigured
// chain. Each one below names the module, the offending address, and the
// exact file to fix.
//
