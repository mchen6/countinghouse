    ...
    cdifDevice.getHWAddress(function(err, addr) {
      ...
      if (!err) {
        ...
        deviceDB.getDeviceUUIDFromHWAddr(hwAddr, function(err, data) {
          if (err) {
            LOG.DE(cdifDevice, new CdifError(err));
            if (options.enableVerifyAndPublish !== true) return;      // fall through in case of we are verifying a module
          }
          if (!data) {
            deviceUUID = uuid.v4();
            deviceDB.setDeviceUUID(hwAddr, deviceUUID, function(err) {
              if (err) {
                LOG.DE(cdifDevice, new CdifError('cannot insert address record for device:' + cdifDevice.spec.device.friendlyName));
                if (options.enableVerifyAndPublish !== true) return;      // fall through in case of we are verifying a module
              }
            });
          } else {
          ...
          LOG.I('new device online: ' + cdifDevice.deviceID);
          _this.deviceMap[deviceUUID] = cdifDevice;
        });
      } else {
        LOG.DE(cdifDevice, new CdifError('cannot get HW address for device: ' + cdifDevice.spec.device.friendlyName));
      }
    });
  });
};
...
DeviceManager.prototype.checkDeviceInterface = function(cdifDevice) {
  if (cdifDevice.lastDeviceError != null) {
    LOG.DE(cdifDevice, new CdifError('lastDeviceError is CDIF reserved field for device object'));
    return false;
  }
