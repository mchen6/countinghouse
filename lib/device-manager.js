        ...
        if (this.allDevicesLoaded === false) {
          if (this.notifyDeviceLoad[uuid] != null) {
            for (var i = 0; i < this.notifyDeviceLoad[uuid].length; i++) {
              ...
              // it could be unsafe and block the whole framework
              cb(null, cdifDevice);
            }
  ...
  this.ensureDeviceState(deviceID, token, function(err, cdifDevice) {
    ...
    session.setDeviceTimer(cdifDevice, function(error, device, timer) {
      try {
        ...
        return this.callback(new DeviceError('DEVICE_ACTION_CALL_FAIL', {message: e.message, stackInfo: e.stack}), null);
      }
    }.bind(session));
  });
};
  ...
  this.ensureDeviceState(deviceID, token, function(err, cdifDevice) {
    ...
    session.setDeviceTimer(cdifDevice, function(error, device, timer) {
      try {
        ...
        return this.callback(new DeviceError('DEVICE_INVOKE_CALLBACK_FAIL', {message: e.message, stackInfo: e.stack}), null);
      }
    }.bind(session));
