...
DeviceManager.prototype.onDeviceOnline = function(cdifDevice, moduleInstance, moduleName) {
  ...
  validator.validateDeviceSpec(cdifDevice.spec, function(error) {
    ...
    UUID.v5({
        ...
        if (options.workerThread === true && !isMainThread) {
          //send heap, cpu statistics data to ccl
          if (options.apiMonitor === true) {
            setInterval(function() {
              this.workerMessage.sendHeapStatisticsMessageToParent({heapStat: v8.getHeapStatistics()});
            }.bind(this), 10000);
          }
        }
...
DeviceManager.prototype.onAllModulesDiscovered = function() {
  ...
  if (options.workerThread !== true && isMainThread === true && options.apiMonitor === true) {
    setInterval(function() {
      ...
      monitor.sendHeapStatMessageToParentController(deviceList, {data: {heapStat: v8.getHeapStatistics()} });
    }.bind(this), 10000);
  }
};
