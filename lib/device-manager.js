...
DeviceManager.prototype.onWorkerLoaded = function(workerMessage) {
  ...
  workerMessage.on('deviceonline', function(msg, wm) {
    ...
    if (deviceID != null) {
      console.log('adding new ref for device: ' + deviceID);
      this.deviceMap[deviceID] = wm; //instead of cdifDevice we save workerMessage instance to deviceMap so we can send msg to it
    }
  }.bind(this));
};
  ...
  if (options.workerThread === true && isMainThread === true) {
    for (var deviceID in this.deviceMap) {
      if (this.deviceMap[deviceID] === moduleInstance) {
        console.log('delete old ref for device: ' + deviceID);
