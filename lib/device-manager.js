...
DeviceManager.prototype.invokeAction = function(cdifDevice, deviceID, serviceID, actionName, args, session) {
  ...
  if (typeof(session) === 'function') {
    try {
      ...
      return session(new DeviceError('DEVICE_INVOKE_EXCEPTION', e.message), null);
    }
  } else {
    session.setDeviceTimer(cdifDevice, function(error, device, timer) {
      try {
        ...
        // console.log('AAAAAAA: ' + err.stack);
        return this.callback(new DeviceError('DEVICE_INVOKE_EXCEPTION', e.message), null);
      }
    }.bind(session));
  }
};
