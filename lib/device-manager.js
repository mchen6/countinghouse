...
DeviceManager.prototype.onDeviceOnline = function(cdifDevice, moduleInstance, moduleName, packageInfo, modulePath) {
  ...
  validator.validateDeviceSpec(cdifDevice.spec, function(error) {
    if (error) {
      ...
      var detail = 'stage=validateDeviceSpec';
      if (Array.isArray(error.validationErrors) && error.validationErrors.length > 0) {
        detail += ' -- ' + error.validationErrors.length + ' schema error(s): ' +
                  error.validationErrors.map(function(e) {
...
DeviceManager.prototype.invokeActionViaPeerChannel = function(appKey, deviceID, serviceID, actionName, args, callback) {
  ...
  if (channel != null && channel.closed !== true) {
    ...
    return channel.invoke(deviceID, serviceID, actionName, args.input, options.requestTimeout, function(err, data, platformMetering) {
...
DeviceManager.prototype.flushPendingPeerInvokes = function(deviceID, workerId, err) {
  ...
  queued.forEach(function(item) {
    ...
    channel.invoke(deviceID, item.serviceID, item.actionName, item.args.input, options.requestTimeout, function(cErr, data, platformMetering) {
  ...
  var channel = new PeerChannel(msg.port, {
    ...
    onInvoke: function(request, reply) {
      ...
      CHUtil.ci.invokeDeviceAction(request.deviceID, request.serviceID, request.actionName, {input: request.input}, null, function(err, data) {
        ...
        }, function(meteringErr, meteringResult) {
...
DeviceManager.prototype.onWorkerLoaded = function(workerMessage) {
  ...
  workerMessage.on('getjobinfo', function(message, wm) {
    ...
    // ownership comment.
    JobControl.getJob(JobControl.INTERNAL, jobID, function(e, job) {
...
DeviceManager.prototype.sendInvokeActionMessageToWorker = function(appKey, deviceID, serviceID, actionName, args, msgID, callerWM) {
  ...
  var localCB = function(err, data, platformMetering) {
    if (err != null) {
  ...
  userAuth(null, null, deviceID, appKey, serviceID, actionName, localCB, function(err, session) {
    ...
    session.setDeviceTimer(calleeWM, function(error, device, timer) {
      ...
      calleeWM.sendInvokeActionMessage({deviceID: deviceID, serviceID: serviceID, actionName: actionName, args: args}, function(err, data) {
        ...
        var tool = encodeLegacyTool(deviceID, serviceID, actionName);

        return CHUtil.ci.recordCall(appKey, tool, options.mcpToolCallCost, function(meteringErr, meteringResult) {
...
DeviceManager.prototype.resolveSchemaInfo = function(cdifDevice, deviceInfo) {
  ...
  var SCHEMA_KEYS  = ['input', 'output', 'fault'];
  ...
  for (var serviceID in serviceList) {
    ...
    for (var i = 0; i < actionList.length; i++) {
      ...
      var loadedAction  = cdifDevice.services[serviceID].actions[action.name];

      for (var k = 0; k < SCHEMA_KEYS.length; k++) {
        var key = SCHEMA_KEYS[k];
