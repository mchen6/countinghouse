...
DeviceManager.prototype.onQueryDeviceList = function(moduleInstance, name, version, callback) {
  var deviceList = [];
  var hasError   = false;
  ...
  for (var deviceID in this.deviceMap) {
    if (this.deviceMap[deviceID].module === moduleInstance) {
      ...
      this.resolveSchemaInfo(deviceInfo.spec);
  ...
  // TODO: add device error information, which may be generated during device interface or schema check here
  if (hasError) {
    return this.moduleManager.emit('querydevicelistresult', new CdifError('module verification failed'), deviceList, name, version, callback);
  }
  return this.moduleManager.emit('querydevicelistresult', null, deviceList, name, version, callback);
};
...
DeviceManager.prototype.resolveSchemaInfo = function(spec) {
