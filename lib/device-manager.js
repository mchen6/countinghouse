...
DeviceManager.prototype.onDeviceOnline = function(cdifDevice, m) {
  ...
  validator.validateDeviceSpec(cdifDevice.spec, function(error) {
    ...
    if (m == null) {
      return console.error('unknown module for device: ' + cdifDevice.spec.device.friendlyName);
    }
...
DeviceManager.prototype.onConnectDevice = function(cdifDevice, user, pass, session) {
  ...
  try {
    cdifDevice.connect(user, pass, function(err, secret, redirectObj) {
      ...
      cdifDevice.clearDeviceTimeout('onconnect', this);
      ...
      if (secret) {
        ...
        _this.deviceAuth.generateToken(user, secret, function(err, token) {
          if (err) {
            return this.callback(new CdifError('cannot generate access token'), null);
          }
          if (redirectObj) {
    ...
    if (cdifDevice.online === false) return;
    cdifDevice.clearDeviceTimeout('onconnect', session);

    return session.callback(new DeviceError(e.message), null);
  }
};
...
DeviceManager.prototype.onDisconnectDevice = function(cdifDevice, session) {
  try {
    cdifDevice.disconnect(function(err) {
      if (cdifDevice.online === false) return;
      cdifDevice.clearDeviceTimeout('ondisconnect', session);
      this.callback(err);
    }.bind(session));
  } catch (e) {
      if (cdifDevice.online === false) return;
...
DeviceManager.prototype.onInvokeDeviceAction = function(cdifDevice, serviceID, actionName, args, session) {
  try {
    cdifDevice.deviceControl(serviceID, actionName, args, function(err, data) {
      if (cdifDevice.online === false) return;
      cdifDevice.clearDeviceTimeout('oninvokeaction', session);
      this.callback(err, data);
    }.bind(session));
  } catch (e) {
    if (cdifDevice.online === false) return;
    cdifDevice.clearDeviceTimeout('oninvokeaction', session);
    return session.callback(new DeviceError(e.message), null); //framework won't throw
  }
};
...
DeviceManager.prototype.onGetDeviceSpec = function(cdifDevice, session) {
  cdifDevice.getDeviceSpec(function(err, data) {
    if (cdifDevice.online === false) return;
    cdifDevice.clearDeviceTimeout('ongetspec', session);
    this.callback(err, data);
  }.bind(session));
};
...
DeviceManager.prototype.onGetDeviceState = function(cdifDevice, serviceID, session) {
  cdifDevice.getServiceStates(serviceID, function(err, data) {
    if (cdifDevice.online === false) return;
    cdifDevice.clearDeviceTimeout('ongetstate', session);
    this.callback(err, data);
  }.bind(session));
};
...
DeviceManager.prototype.onEventSubscribe = function(subscriber, cdifDevice, serviceID, session) {
  cdifDevice.subscribeDeviceEvent(subscriber, serviceID, function(err) {
    if (cdifDevice.online === false) return;
    cdifDevice.clearDeviceTimeout('onsubscribe', session);
    this.callback(err);
  }.bind(session));
};
...
DeviceManager.prototype.onEventUnsubscribe = function(subscriber, cdifDevice, serviceID, session) {
  cdifDevice.unSubscribeDeviceEvent(subscriber, serviceID, function(err) {
    if (cdifDevice.online === false) return;
    cdifDevice.clearDeviceTimeout('onunsubscribe', session);
    this.callback(err);
  }.bind(session));
};
...
DeviceManager.prototype.onSetDeviceOAuthAccessToken = function(cdifDevice, params, session) {
  cdifDevice.setOAuthAccessToken(params, function(err) {
    if (cdifDevice.online === false) return;
    cdifDevice.clearDeviceTimeout('onsetoauthtoken', session);
    this.callback(err);
  }.bind(session));
};
...
DeviceManager.prototype.checkDeviceInterface = function(cdifDevice) {
  if (!(cdifDevice instanceof CdifDevice)) {
    console.error('object is not an instance of CDIF device');
    return false;
  }
  if (typeof(cdifDevice._connect) !== 'function' || typeof(cdifDevice._disconnect) !== 'function' || typeof(cdifDevice._getHWAddress) !== 'function') {
    console.error('device interface mismatch');
    return false;
  }
  return true;
};
