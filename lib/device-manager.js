...
DeviceManager.prototype.onConnectDevice = function(deviceID, user, pass, session) {
  ...
  var cdifDevice = this.deviceMap[deviceID];
  if (cdifDevice == null) {
    return session.callbackWithoutTimer(new CdifError('device not found: ' + deviceID));
  }

  if (cdifDevice.module.discoverState === 'discovering') {
    return session.callbackWithoutTimer(new CdifError('in discovering', null));
  }
...
DeviceManager.prototype.onEventSubscribe = function(subscriber, deviceID, serviceID, token, session) {
  this.ensureDeviceState(deviceID, token, function(err, cdifDevice) {
    if (err) return session.callbackWithoutTimer(err, null);

    session.setDeviceTimer(cdifDevice, function(error, device, timer) {
      device.subscribeDeviceEvent(subscriber, serviceID, session);
    }.bind(session));
  });
};

DeviceManager.prototype.onEventUnsubscribe = function(subscriber, deviceID, serviceID, token, session) {
  this.ensureDeviceState(deviceID, token, function(err, cdifDevice) {
    if (err) return session.callbackWithoutTimer(err, null);

    session.setDeviceTimer(cdifDevice, function(error, device, timer) {
      device.unSubscribeDeviceEvent(subscriber, serviceID, session);
    }.bind(session));
  });
};
