...
DeviceManager.prototype.verifyComposition = function(deviceID, spec, packageInfo, modulePath, callback) {
  ...
  if (friendlyName == null) {
    return callback(new CHError('MODULE_COMPOSITION_INVALID', String(moduleName),
  ...
  if (moduleName !== friendlyName) {
    return callback(new CHError('MODULE_COMPOSITION_INVALID', moduleName,
      `package.json name "${moduleName}" (${modulePath}/package.json) and api.json friendlyName ` +
      `"${friendlyName}" (${modulePath}/api.json) disagree. Composition binds by friendlyName, so ` +
      'they must match.'));
  }
  ...
  authProvider.identityForModule(friendlyName, (identErr, identity) => {
    ...
    if (identity.conflicts != null) {
      return callback(new CHError('MODULE_COMPOSITION_INVALID', friendlyName,
        ...
        identity.conflicts.join(', ')}. Exactly one must -- edit your auth config so only one does.`));
    }
    if (identity.apiKey == null) {
      return callback(new CHError('MODULE_COMPOSITION_INVALID', friendlyName,
        `it declares ${declared.length} composition target(s) in ${modulePath}/package.json's ` +
          ...
          (authErr, result) => {
            ...
            if (result.ok !== true) {
              return finish(new CHError('MODULE_COMPOSITION_INVALID', friendlyName,
                ...
                targetID} (module "${parsed.device}") to its "devices" in your auth config.`));
            }
