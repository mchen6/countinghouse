...
DeviceManager.prototype.onEventSubscribe = function(subscriber, deviceID, serviceID, actionName, input, inputKey, token, callback) {
  this.ensureDeviceState(deviceID, token, function(err, cdifDevice) {
    if (err) return callback(err);
    cdifDevice.subscribeDeviceEvent(subscriber, serviceID, actionName, input, inputKey, callback);
  });
};

DeviceManager.prototype.onEventUnsubscribe = function(subscriber, deviceID, serviceID, actionName, input, inputKey, token, callback) {
  this.ensureDeviceState(deviceID, token, function(err, cdifDevice) {
    if (err) return callback(err);
    cdifDevice.unsubscribeDeviceEvent(subscriber, serviceID, actionName, input, inputKey, callback);
  });
};
