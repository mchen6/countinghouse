...
var McpForgeDevice  = require('./mcpforge-device');
...
var McpForgeUtil    = require('./mcpforge-util');
...
if (isMainThread === true) {
  ...
  redisClient.on('error', function (err) {
    if (options.debug !== true) LOG.E(new McpForgeError('REDIS_CLIENT_ERROR', err.message));
  });
}
...
DeviceManager.prototype.onDeviceOnline = function(cdifDevice, moduleInstance, moduleName, packageInfo, modulePath) {
  ...
  validator.validateDeviceSpec(cdifDevice.spec, function(error) {
    if (error) {
      LOG.DE(cdifDevice, new McpForgeError('DEVICE_SPEC_VALIDATION_FAIL', error.message));
      if (options.verifyModule !== true) return;       // fall through in case of we are verifying a module
    }
    if (moduleInstance == null) {
      LOG.DE(cdifDevice, new McpForgeError('INVALID_MODULE_INSTANCE', cdifDevice.spec.device.friendlyName));
      if (options.verifyModule !== true) return;      // fall through in case of we are verifying a module
    }

    // deviceID is generated in McpForgeDevice's constructor code
    // and should be already available here
    var uuid = cdifDevice.deviceID;
    if (uuid == null) {
      LOG.DE(cdifDevice, new McpForgeError('DEVICE_ID_INVALID_OR_NULL'));
      return;
    }

    if(this.deviceMap[uuid] != null) {
      if (this.deviceMap[uuid].module === moduleInstance) {
        LOG.DE(cdifDevice, new McpForgeError('DEVICE_OBJECT_CONFLICT'));
        if (options.verifyModule !== true) return;      // fall through in case of we are verifying a module
      }
    }
...
DeviceManager.prototype.createRateLimiterForDevice = function(cdifDevice) {
  ...
  if (cdifDevice instanceof McpForgeDevice) {
...
DeviceManager.prototype.onWorkerLoaded = function(workerMessage) {
  ...
  workerMessage.on('devicelog', function(message, wm) {
    ...
    McpForgeUtil.__deviceLogWithID(deviceID, data, timestamp, function(e) {
...
DeviceManager.prototype.onDeviceOffline = function(cdifDevice, moduleInstance) {
  LOG.DE(cdifDevice, new McpForgeError('DEVICE_OFFLINE', cdifDevice.spec.device.friendlyName));
  ...
  if (options.workerThread === true && isMainThread === true) {
    for (var deviceID in this.deviceMap) {
      if (this.deviceMap[deviceID] === moduleInstance) {
        ...
        McpForgeUtil.__deviceLogWithID(deviceID, reason, Date.now(), function(){});
...
DeviceManager.prototype.onQueryDeviceList = function(moduleInstance, packageInfo, callback) {
  ...
  if (hasError) {
    return this.moduleManager.emit('querydevicelistresult', new McpForgeError('MODULE_VERIFICATION_FAILED'), deviceList, packageInfo, callback);
  }
...
DeviceManager.prototype.ensureDeviceState = function(deviceID, token, callback) {
  ...
  if (options.workerThread === true && isMainThread === true) {
    ...
    if (!(cdifDevice instanceof WorkerMessage)) return callback(new McpForgeError('DEVICE_NOT_FOUND', deviceID));
  }

  if (cdifDevice == null) {  // check null or undefined
    return callback(new McpForgeError('DEVICE_NOT_FOUND', deviceID));
  }

  if (cdifDevice.online === false) {
    return callback(new McpForgeError('DEVICE_OFFLINE'));
  }
...
DeviceManager.prototype.invokeJobs = function(cdifDevice, deviceID, serviceID, actionName, input, jobID, callback) {
  try {
    if (options.workerThread === true && isMainThread === true) {
      ...
      return callback(new McpForgeError('DEVICE_ACTION_CALL_FAIL', 'job cannot be processed in non-worker mode, please restart server in multi-thread mode. this job will fail.'));
    }
  } catch (e) {
    return callback(new McpForgeError('DEVICE_ACTION_CALL_FAIL', e.message), null);
  }
};
...
DeviceManager.prototype.onInvokeJobs = function(deviceID, serviceID, actionName, input, jobID, callback) {
  this.ensureDeviceState(deviceID, null, function(err, cdifDevice) {
    ...
    if (isMainThread !== true) return callback(new McpForgeError('DEVICE_ACTION_CALL_FAIL', 'cannot start process job in child thread, this job will fail'));
...
DeviceManager.prototype.checkDeviceInterface = function(cdifDevice) {
  ...
  if (!(cdifDevice instanceof McpForgeDevice)) {
    LOG.DE(cdifDevice, new McpForgeError('DEVICE_OBJECT_NOT_VALID_CDIF_DEVICE'));
    return false;
  }
  return true;
};
