  ...
  for (var serviceID in serviceList) {
    ...
    for (var actionName in actionList) {
      ...
      for (var argumentName in action.argumentList) {
        ...
        if (stateVariable.dataType !== 'object') { // XXXX this assumption is only valid for API management
          deviceInfo.deviceErrorMessage = 'state variable dataType must be object';
          return true;
        }
