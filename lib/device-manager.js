...
DeviceManager.prototype.onQueryDeviceList = function(moduleInstance, name, version, callback) {
  ...
  for (var deviceID in this.deviceMap) {
    if (this.deviceMap[deviceID].module === moduleInstance) {
      ...
      var deviceInfo = {};
      deviceInfo.spec = cdifDevice.spec;
      deviceList.push(deviceInfo);
    }
  }
  // TODO: add device error information, which may generated during device interface or schema check here
  this.moduleManager.emit('querydevicelistresult', null, deviceList, name, version, callback);
};
...
DeviceManager.prototype.checkDeviceInterface = function(cdifDevice) {
  ...
  if (typeof(cdifDevice._connect) !== 'function' || typeof(cdifDevice._disconnect) !== 'function' || typeof(cdifDevice._getHWAddress) !== 'function') {
    logger.error('device interface mismatch');
    return false;
  }
  return true;
};
