...
DeviceManager.prototype.onWorkerLoaded = function(workerMessage) {
  ...
  workerMessage.on('rediscommand', function(message, wm) {
    ...
    redisAPICache.client[opName](data, function(e, result) {
...
DeviceManager.prototype.invokeJobs = function(cdifDevice, deviceID, serviceID, actionName, input, callback) {
  try {
    if (options.workerThread === true && isMainThread === true) {
      cdifDevice.sendInvokeActionMessage({deviceID: deviceID, serviceID: serviceID, actionName: actionName, args: {input: input}}, function(err, result) {
        if (err) return callback(new DeviceError(err.message), result);
        return callback(null, result);
      });
    } else {
      return callback(new CdifError('DEVICE_ACTION_CALL_FAIL', 'job cannot be processed in non-worker mode, this job will fail, please add this job again'));
    }
  } catch (e) {
...
DeviceManager.prototype.onInvokeJobs = function(deviceID, serviceID, actionName, input, callback) {
  this.ensureDeviceState(deviceID, null, function(err, cdifDevice) {
    if (err) return callback(err);
    console.log('xxx');
    if (isMainThread !== true) return callback(new CdifError('DEVICE_ACTION_CALL_FAIL', 'cannot start process job in child thread, this job will fail, please add job again'));
    return this.invokeJobs(cdifDevice, deviceID, serviceID, actionName, input, callback);
  }.bind(this));
}
