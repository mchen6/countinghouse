...
PeerChannelBroker.prototype.attachToWorker = function(workerMessage) {
  ...
  workerMessage.on('peer-metering-request', function(message, wm) {
...
PeerChannelBroker.prototype.handleMeteringRequest = function(data, msgID, wm) {
  ...
  var tool = encodeLegacyTool(data.deviceID, data.serviceID, data.actionName);

  CHUtil.ci.recordCall(data.callerModule, tool, options.mcpToolCallCost, function(err, result) {
    if (err) {
      LOG.E(err);
