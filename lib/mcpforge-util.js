...
module.exports = {
  ...
  inherits: function(constructor, superConstructor) {
    ...
    if (superConstructor === McpForgeDevice) {
  ...
  invokeAction: function(userKey, deviceBaseUrl, serviceID, actionName, args, callback) {
    // convenience method to invoke a MCPForge action
  ...
  createServiceClient: function(opts, callback) {
    ...
    //under debug mode we query remote server, e.g. api.mcpforge.com:3049/device-list to get the serviceID
  ...
  deviceLog: function(device, entry) {
    if (!(device instanceof McpForgeDevice)) return;
