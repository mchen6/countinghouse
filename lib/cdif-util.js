...
module.exports = {
  ...
  createServiceClient: function(opts, callback) {
    ...
    if (options.isDebug === true && options.enableVerifyAndPublish === true) {
      var deviceListOpts = {
        url: 'https://api.apemesh.com:3049/device-list',
        method: 'GET',
        headers: {
        ...
        async.each(info, function(item, cb) {
          if (item.device.deviceID === deviceID) {
            var getDeviceSpecOpts = {
              //FIXME: configurable api entry address
              url: 'https://api.apemesh.com:3049/devices/' + item.device.deviceID + '/get-spec',
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'X-Apemesh-Key': appKey
            ...
            request(getDeviceSpecOpts, function(err, response, body) {
              var spec = null;
              if (err != null) {
                return cb(err);
              }
              ...
              if (found === true) {
                ...
                //FIXME: configurable api entry address
                targetService.deviceBaseUrl = 'https://api.apemesh.com:3049/devices/' + item.device.deviceID;
                return cb();
              }
      ...
      // FIXME: check existence of the serviceID inside device object
      this.dm.emit('querydevice', deviceID, function(error, cdifDevice) {
        if (error != null) return callback(new Error(error.message), null);
        return callback(null, new ServiceClient(this.ci, false, appKey, deviceID, serviceID));
      }.bind(this));
    }
  }
