...
OAuthDevice.prototype.setOAuthAccessToken = function(params, callback) {
  if (this.oauth_version === '1.0') {
    ...
    params.redirect_uri = CdifUtil.getHostProtocol() + '127.0.0.1' + ':' + options.port + '/callback_url';
...
OAuthDevice.prototype.connect = function(user, pass, callback) {
  if (this.oauth_version === '1.0') {
    ...
    var requestUrl = this.oauth_requestUrl + '?oauth_callback=' + querystring.escape(CdifUtil.getHostProtocol() + CdifUtil.getHostIp() + ':' + options.port + '/callback_url?deviceID=' + this.deviceID);
  ...
  if (this.oauth_version === '1.0') {
    ...
    // for oauth 2.0 use 127.0.0.1 instead of real host address
    authorize_params.redirect_uri = CdifUtil.getHostProtocol() + '127.0.0.1' + ':' + options.port + '/callback_url';
