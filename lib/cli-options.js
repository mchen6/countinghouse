// do not require McpForgeUtil here to avoid circular dep
...
module.exports = {
  setOptions: function(argv) {
    ...
    this.vscode                  = (argv.vscode          === true) ? true : false;

    this.workerThread            = (argv.workerThread    === true) ? true : false;
    this.withPM2                 = (argv.withPM2         === true) ? true : false;

    this.modulePath   = argv.modulePath || process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'] + '/mcpforge_modules';
    ...
    // the entry url of mcpforge, it should be configured to local url for on-premise deployments
    this.centralPortalUrl = argv.centralPortalUrl || 'https://api.mcpforge.com:3049';
