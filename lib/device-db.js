...
module.exports = {
  ...
  setModuleInfo: function(name, version, path, callback) {
    if (options.localDBAccess === false) return callback(null);
    ...
    this.moduleDB.serialize(function() {
      this.moduleDB.run("INSERT OR REPLACE INTO module_info(name, version, path) VALUES (?, ?, ?)",
      name, version, path, callback);
    }.bind(this));
  },
  ...
  init: function() {
    ...
    if (options.localDBAccess === false) {
      return LOG.I('local device DB access is disabled');
    }

    if (options.localDBPath !== null) {
      deviceDBName = options.localDBPath + '/device_store.db';
      moduleDBName = options.localDBPath + '/modules.db';
      //TODO: check write safety of this call, do not crash
      try {
        mkdirp.sync(options.localDBPath);
        fs.accessSync(options.localDBPath, fs.W_OK);
      } catch (e) {
        return LOG.E(new CdifError('cannot access DB folder: ' + options.localDBPath + ' reason: ' + e.message));
        process.exit(-1);
      }
    } else {
    ...
    this.moduleDB.serialize(function() {
      this.moduleDB.run("CREATE TABLE IF NOT EXISTS module_info(name TEXT PRIMARY KEY, version TEXT, path TEXT)");
    }.bind(this));
  }
};
