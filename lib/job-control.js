...
module.exports = {
  ...
  initJobProcess: function(cdifInterface) {
    ...
    this.jobQueue.process("*", 1024, function(job, done) {
  ...
  addJob: function(jobOpts, deviceID, serviceID, actionName, input, callback) {
    if (options.workerThread !== true) {
      return callback(new McpForgeError('ADD_JOB_ERROR', 'mcpforge should be in multi-thread mode'));
    }
    if (jobOpts == null) {
      return callback(new McpForgeError('ADD_JOB_ERROR', 'must have opts to add a job'));
    }
    if (jobOpts.name == null) {
      return callback(new McpForgeError('ADD_JOB_ERROR', 'job must have a name'));
    }
    ...
    this.jobQueue.add(name, {
      ...
      return callback(new McpForgeError('ADD_JOB_ERROR', err.message));
    });
  },

  getJob: function(id, callback) {
    if (options.workerThread !== true) {
      return callback(new McpForgeError('GET_JOB_ERROR', 'mcpforge should be in multi-thread mode'));
    }

    this.jobQueue.getJob(id).then(function(job) {
      if (job == null) return callback(new McpForgeError('GET_JOB_ERROR', 'unknown job'));
      ...
      job.getState().then(function(state) {
        ...
        return callback(new McpForgeError('GET_JOB_ERROR', error.message));
      });
    }).catch(function(err) {
      return callback(new McpForgeError('GET_JOB_ERROR', err.message));
    });
  },
  ...
  removeJob: function(name, jobID, isRepeat, callback) {
    ...
    if (options.workerThread !== true) {
      return callback(new McpForgeError('REMOVE_JOB_ERROR', 'mcpforge should be in multi-thread mode'));
    }
    if (name == null) {
      return callback(new McpForgeError('REMOVE_JOB_ERROR', 'job must have a name'));
    }
    if (isRepeat == null ||typeof(isRepeat) !== 'boolean') {
      return callback(new McpForgeError('REMOVE_JOB_ERROR', 'must pass in isRepeat boolean flag'));
    }

    if (isRepeat === false) {
      _this.jobQueue.getJob(jobID).then(function(job) {
        if (job == null) return callback(new McpForgeError('REMOVE_JOB_ERROR', 'unknown job'));
        job.remove();
        return callback(null, {removed: true});
      }).catch(function(err) {
        return callback(new McpForgeError('REMOVE_JOB_ERROR', err.message));
      });
    } else {
      var found = false;
      ...
      _this.jobQueue.getRepeatableJobs().then(function(repeatable) {
        ...
        return callback(new McpForgeError('REMOVE_JOB_ERROR', 'unknown job'));
      }).catch(function(err) {
        return callback(new McpForgeError('REMOVE_JOB_ERROR', err.message));
      });
    }
