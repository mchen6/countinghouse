      ...
      for (var actionName in actionList) {
        ...
        if (options.allowSimpleType !== true) {
          if (Object.keys(action.argumentList).length > 2) {
            return callback(new Error('argumentList cannot contain more than two arguments. Service: ' + serviceID + ', action: ' + actionName));
          }
        }

        for (var argumentName in action.argumentList) {
          console.log(argumentName);
          if (options.allowSimpleType !== true && (argumentName !== 'input' && argumentName !== 'output')) {
            return callback(new Error('argument name must be either \'input\' or \'output\'. Service: ' + serviceID + ', action: ' + actionName + ', argument: ' + argumentName));
          }
