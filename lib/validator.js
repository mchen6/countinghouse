...
module.exports = {
  ...
  validate: function(name, schemaObj, data, callback) {
    ...
    if (data === null) {
      ...
      if (schema == null) {   // check both null and undefined
        ...
        var validator = schemaObj.validator;
        if (validator == null) {
  ...
  detectLegacySpec: function(spec) {
    ...
    var serviceList = (spec.device != null) ? spec.device.serviceList : null;
  ...
  validateDeviceSpec: function(spec, callback) {
    ...
    var legacy = this.detectLegacySpec(spec);
    if (legacy != null) {
    ...
    for (var serviceID in serviceList) {
      ...
      for (var i = 0; i < actionList.length; i++) {
        var actionName = actionList[i].name;
