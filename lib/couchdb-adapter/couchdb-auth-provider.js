...
// balance, devices] where devices = [{deviceID, priceRecord}, ...] --
...
// AuthProvider's (see provider.js's header comment).
//
...
function CouchDBAuthProvider(opts) {
  ...
  this._cache = {}; // apiKey -> {userName, devices}
}
...
CouchDBAuthProvider.prototype._lookup = function(apiKey, callback) {
  ...
  this.usersDB.view('user', 'byAppKeyWithBasicInfo', {key: apiKey}, function(err, doc) {
    ...
    var entry = {userName: userName, devices: devices};
