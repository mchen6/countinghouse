...
// refactor -- see docs/design-decisions.md's AuthProvider
...
CouchDBAuthProvider.prototype._lookup = function(apiKey, callback) {
  ...
  this.usersDB.view('user', 'byAppKeyWithBasicInfo', {key: apiKey}, (err, doc) => {
    if (err) return callback(err);
    if (doc.rows.length === 0) return callback(null, null);

    const userName    = doc.rows[0].value[0];
    const devicesRaw  = doc.rows[0].value[2];
    const adminRaw    = doc.rows[0].value[3];
    let devices = [];
    if (Array.isArray(devicesRaw)) {
      devices = devicesRaw.map((d) => { return d.deviceID; });
    }

    const entry = {userName: userName, devices: devices, admin: adminRaw === true};
