...
function parseArgs(argv) {
  let dbUrl = 'http://admin:12345678@127.0.0.1:5984';
...
const byAppKeyWithBasicInfoMap = function(doc) {
...
function main() {
  ...
  const server = nano(opts.dbUrl);

  server.db.create('_users', (err) => {
    ...
    const usersDB = nano(opts.dbUrl).db.use('_users');

    usersDB.get('_design/user', (getErr, existing) => {
      const doc = JSON.parse(JSON.stringify(DESIGN_DOC));
