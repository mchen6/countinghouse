...
// opts.billingKey overrides this.billingKey for one call. That exists so a
// client can be *reused* across callers: ctx.serviceClient hands out a cached
// client (creating one per call allocates a ServiceClient, a QueryDevice and a
// device lookup on every invoke, which showed up as the benchmark exhausting
// the process) while each call still bills whoever actually made it. Passing
// it per call rather than mutating this.billingKey is deliberate -- mutation
// would race between concurrent callers sharing the cached client.
ServiceClient.prototype.invoke = function(opts, callback) {
  ...
  if (this.isRemote === true) {
    ...
    const billingKey = (opts.billingKey != null) ? opts.billingKey : this.billingKey;
    ...
    if (this.isRemoteThread === true) {
      ...
      if (options.directPeerChannels === true) {
        return dm.invokeActionViaPeerChannel(this.appKey, billingKey, this.deviceID, this.serviceID, opts.actionName, args, callback);
      }
      return dm.sendActionInvokeMessageToParent(this.appKey, billingKey, this.deviceID, this.serviceID, opts.actionName, args, callback);
    }
    ...
    userAuth(null, null, this.deviceID, this.appKey, this.serviceID, opts.actionName, callback, (err, session) => {
      ...
      this.cdifInterface.invokeDeviceAction(this.deviceID, this.serviceID, opts.actionName, args, session);
    });
  }
}
