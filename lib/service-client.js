...
ServiceClient.prototype.invoke = function(opts, callback) {
  ...
  if (this.isRemote === true) {
    ...
    //TODO: under worker mode session obj may contain a workerMessage instance so we can send msgs to it on invoke service actions
    // but make sure that msg is not reponded through parentPort, instead a port locally to the sender itself
