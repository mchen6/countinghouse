...
function CHDevice(spec) {
  ...
  // 'apemesh' here is a deliberately kept hash seed, not a missed rename --
  // changing it would reassign every existing device's UUID on upgrade.
...
CHDevice.prototype.getServiceStates = function(serviceID, session) {
  if (session == null) {
...
CHDevice.prototype.setServiceStates = function(serviceID, values, callback) {
  if (callback == null || typeof(callback) !== 'function') {
    return LOG.DE(this, new CHError('GET_SERVICE_STATE_FAIL_INVALID_CALLBACK'));
  }
  var service = this.services[serviceID];
  if (service == null) {
...
CHDevice.prototype.setEventSubscription = function(serviceID, subscribe, unsubscribe) {
...
CHDevice.prototype.subscribeDeviceEvent = function(serviceID, actionName, input, inputKey, callback) {
  var service = this.services[serviceID];
  if (service == null) {
...
CHDevice.prototype.unsubscribeDeviceEvent = function(serviceID, actionName, input, inputKey, callback) {
  var service = this.services[serviceID];
  if (service == null) {
    return callback(new DeviceError('SERVICE_NOT_FOUND', serviceID));
  }
