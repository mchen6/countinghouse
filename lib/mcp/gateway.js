...
// Bound for polling buildToolTargets' diff after countinghouse_load_module
// (see the comment at its call site for why a single synchronous read isn't
// enough under --workerThread). Generous relative to the actual round trip
// (a worker message hop, not real I/O), but bounded so a module whose device
// never comes online doesn't hang the tool call indefinitely -- it still
// answers `loaded: true, toolNames: []` once the deadline passes, which is
// the honest answer for a load that succeeded but produced no discoverable
// device.
const LOAD_MODULE_DISCOVERY_TIMEOUT_MS = 3000;
const LOAD_MODULE_DISCOVERY_POLL_MS    = 50;

function waitForAddedToolNames(cdifInterface, before, deadline, callback) {
  const after = Object.keys(toolRegistry.buildToolTargets(cdifInterface));
  const added = after.filter((n) => before.indexOf(n) === -1);
  if (added.length > 0 || Date.now() >= deadline) return callback(added);
  return setTimeout(() => waitForAddedToolNames(cdifInterface, before, deadline, callback), LOAD_MODULE_DISCOVERY_POLL_MS);
}

// Every authoring tool takes the same two gates: the flag, then admin. These
// gates are load-bearing today, not preparation for the load_module/
// call_tool tools Tasks 4/5 add -- countinghouse_validate_module, the one
// tool live right now, already does require(callerSuppliedPath) inside
// lib/module-validator.js's loadPackage, unsandboxed, in this main gateway
// process (lib/sandbox.js is not involved). That is arbitrary JS execution
// with a friendly name; --authoringTools plus an admin key is what stands
// between a caller and it, not a UX nicety over an already-safe tool.
//
    ...
    if (name === 'countinghouse_load_module') {
      ...
      return cdifInterface.deviceManager.moduleManager.loadModuleFromPath(
        toolArgs.path, toolArgs.name, toolArgs.version || null, (loadErr) => {
          ...
          // spec, not from anything the caller passed in. But under
          // --workerThread, loadModuleFromPath's callback fires the moment the
          // *load* is acked, not once the device is actually discovered:
          ...
          // populates deviceManager.deviceMap -- so reading the diff
          // synchronously here would almost always see zero added tools.
          // There is no single reliable "discovery finished for this module"
          // event to await instead: module-manager.js's own
          // 'allmodulediscovered' bookkeeping only fires when
          // noofTotalModules > 0, which is false for a module loaded into a
          // server that started with none preloaded. So the diff is polled
          // briefly rather than trusted on the first read.
          return waitForAddedToolNames(cdifInterface, before, Date.now() + LOAD_MODULE_DISCOVERY_TIMEOUT_MS, (added) => {
    ...
    if (name === 'countinghouse_call_tool') {
      ...
      return handleToolsCall(innerReq, cdifInterface, appKey, (innerCallbackErr, innerResp) => {
        ...
        if (innerResp != null && innerResp.error != null) {
          return callback(innerCallbackErr, resultResponse(req.id, toolCallResult(new Error(innerResp.error.message))));
        }
