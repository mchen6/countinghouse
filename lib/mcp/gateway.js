...
// call_tool tools Tasks 4/5 add -- countinghouse_validate_module runs
// require(callerSuppliedPath) (inside lib/module-validator.js's loadPackage)
// to do its job, unsandboxed (lib/sandbox.js is not involved). That is
// arbitrary JS execution with a friendly name; --authoringTools plus an
// admin key is what stands between a caller and it, not a UX nicety over an
// already-safe tool. As of Task 8, that require() happens in a spawned
// child process (see validateModuleInChildProcess below) rather than this
// one, which closes two problems that in-process execution had regardless
// of the auth gate: a require() cache that went stale across repeated
// validate calls on an edited module, and a module whose entry called
// process.exit() taking the whole gateway down with it. The gate below is
// still what an attacker without a flag+admin key has to get through in the
// first place; the child process is what keeps a legitimate caller's own
// module-under-test from corrupting or killing the server that's validating it.
//
...
// caller-supplied JS into this gateway process directly -- see the comment
// on dispatchAuthoringTool below for why. `callback(err, result)` mirrors
// lib/module-validator.js's own validateModule signature exactly: err set
// only for the "path is unusable" case (mirrors the CLI's exit code 2),
// otherwise result is the {ok, module, problems} shape unit tests and the
// tool's outputSchema already expect. A module that crashes or hangs its own
// validator is reported as `result.ok === false` with a problem describing
// that -- not as an error -- since the *validate* succeeded at its job
// (telling the caller something is wrong with the module); it just had to
