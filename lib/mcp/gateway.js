...
function handleToolsCall(req, cdifInterface, appKey, callback) {
  ...
  if (params.task != null) {
    return createTaskForToolCall(req, target, toolArgs, callback);
  }
...
function createTaskForToolCall(req, target, toolArgs, callback) {
  if (tasksSupported() !== true) {
  ...
  // deterministic identifier tools/list and tools/call agree on.
...
function handleTasksResult(req, cdifInterface, callback) {
  ...
  JobControl.getJob(taskId, function(err, record) {
    ...
    // structuredContent contract either way.
    if (status === 'completed') {
      var data = record.job.returnvalue;
      return callback(null, resultResponse(req.id, {
        isError: false,
        content: [{type: 'text', text: JSON.stringify(data == null ? {} : data)}],
        structuredContent: (data == null ? {} : data)
    ...
    if (status === 'failed') {
      return callback(null, resultResponse(req.id, {
        isError: true,
        content: [{type: 'text', text: (record.job.failedReason != null ? record.job.failedReason : 'task failed')}]
      }));
    }
