...
module.exports = {
  ...
  startVSCode: function(workingDir, apiDesignID, callback) {
    ...
    if (this.monitor != null) {
      this.monitor.stop(function() {
        this.monitor = respawn(['/usr/bin/code-server', '--disable-telemetry', '--user-data-dir', '/root/countinghouse-workspace/.UserData', '--extensions-dir', '/root/countinghouse-workspace/.ExtensionData', workingDir], {
  ...
  installWatcherEventHandler: function(apiDesignID, watcher, workingDir) {
    ...
    watcher.on('add', function(codePath) {
      ...
      //do not add dependent packages or .git folder
      if (codePath.startsWith('node_modules') || codePath.startsWith('.git')) return;
