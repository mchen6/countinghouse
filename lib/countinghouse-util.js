...
module.exports = {
  ...
  // Lets a device module record a MeteringProvider call for a
  // ServiceClient.invoke() hop it just made (see createServiceClient above
  // and lib/metering/ -- this is a thin passthrough to
  // CdifInterface.prototype.recordCall, reachable here because
  // countinghouse-interface.js sets `cdifUtil.ci = this` on construction,
  // and now constructs a working meteringProvider in every worker thread,
  // not just the main thread).
  recordCall: function(apiKey, tool, cost, callback) {
    if (this.ci == null) return callback(new Error('CHUtil.ci not available'));
    return this.ci.recordCall(apiKey, tool, cost, callback);
  },
