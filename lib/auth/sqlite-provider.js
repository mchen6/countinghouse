...
// sqlite3 is an optionalDependency: it carries a prebuilt native binding
// that isn't available (or isn't loadable) on every supported host -- the
// prebuild for current versions links against a newer glibc than some
// still-supported distros ship, which surfaces as ERR_DLOPEN_FAILED at
// require time rather than at install time. Required lazily and reported
// clearly, exactly like nano in lib/couchdb-adapter/couchdb-auth-provider.js,
// so a deployment that never selects --authProvider sqlite is unaffected by
// its absence and one that does gets told why rather than a raw loader stack.
function requireSqlite3() {
  try {
    return require('sqlite3');
  } catch (e) {
    throw new Error('SqliteAuthProvider requires the "sqlite3" package, which is not installed or ' +
                    'cannot be loaded on this host (' + e.message.split('\n')[0] + '). ' +
                    'Run: npm install sqlite3 -- or use --authProvider file, which needs nothing.');
  }
}
...
function SqliteAuthProvider(opts) {
  ...
  this.db = opts.db || new (requireSqlite3().Database)(this.dbPath);
  this._ensureSchema();
}
...
SqliteAuthProvider.prototype.authenticate = function(apiKey, deviceID, serviceID, actionName, callback) {
  ...
  this.db.get('SELECT userName, admin FROM users WHERE apiKey = ?', [apiKey], function(err, userRow) {
