...
var InputKeyClass     = require('./input-key');
...
Service.prototype.updateStateFromAction = function(action, input, output, callback) {
  ...
  if (options.enableAPICache === true && action.apiCache != null) {
    ...
    var outputHashString = stringHash(outputString).toString();

    redisClient.hmget(hashString, 'deviceID', 'valueHash', function(err, results) {
      if (err) {
        LOG.E(new CdifError('REDIS_CLIENT_ERROR', err.message));
      ...
      if (results[0] == null) {
        ...
        redisClient.hmset(hashString, 'deviceID', this.device.deviceID, 'value', outputString, 'valueHash', outputHashString, 'timestamp', Date.now(), function(err) {
          this.apiKeyStats[hashString].refreshing = false;
          if (err) {
            LOG.E(new CdifError('REDIS_CLIENT_ERROR', err.message));
            return callback(false);
          }
        ...
        if(results[1] !== outputHashString) {
          redisClient.hmset(hashString, 'value', outputString, 'valueHash', outputHashString, 'timestamp', Date.now(), function(err) {
            this.apiKeyStats[hashString].refreshing = false;
            if (err) {
              LOG.E(new CdifError('REDIS_CLIENT_ERROR', err.message));
              return callback(false);
...
Service.prototype.invokeAction = function(actionName, input, session) {
  ...
  if (options.enableAPIMonitor === true) {
    if (action.realPrice > 0 && session.apiRemainCount <= 0 && session.balance < action.realPrice) {
...
Service.prototype.getApiInputKeyAccessInfo = function(key) {
  ...
  if (this.apiKeyStats[key] == null) {
    ...
    this.apiKeyStats[key].keyObject  = new InputKeyClass(key); // the event emission object
...
Service.prototype.getValueFromAPICache = function(actionName, timeout, input, callback) {
  ...
  redisClient.hmget(hashString, 'deviceID', 'value', 'timestamp', function(err, results) {
    if (err) {
      LOG.E(new CdifError('REDIS_CLIENT_ERROR', err.message));
    ...
    if (empty === true) return callback(null, null, null);
    // in case of hash collision, run the slowest path
    if (results[0] !== this.device.deviceID) return callback(null, null, null);

    var freq = this.getApiInputKeyAccessInfo(hashString);

    var cacheTimeStamp = results[2];
    if (Date.now() - cacheTimeStamp > timeout) {
      if (this.apiKeyStats[hashString].refreshing === false) {
        ...
        this.apiKeyStats[hashString].refreshing = true;
      ...
      this.validateActionCall(action, output, false, function(error, data) {
        ...
        this.updateStateFromAction(action, input, output, function(updated) {
          if (options.enableAPICache === true && action.apiCache != null && updated === true) {
            var hashString = hashKey.getInputHashKey(this.device.deviceID, this.serviceID, action.name, input);
            if (this.apiKeyStats[hashString] != null && this.apiKeyStats[hashString].keyObject != null) {
              LOG.I('value invalidated');
              this.apiKeyStats[hashString].keyObject.emitEvent(JSON.stringify(output));
            }
          }
        }.bind(this));
...
Service.prototype.subscribeEvent = function(subscriber, actionName, input, inputKey, callback) {
  var action = this.actions[actionName];

  if (action == null) {
    return callback(new CdifError('ACTION_NOT_FOUND', actionName), null);
  }
  if (input == null) {
    return callback(new CdifError('INPUT_NOT_FOUND'), null);
  }
  if (action.invoke == null) {
    return callback(new DeviceError('ACTION_NOT_IMPLEMENTED', actionName), null);
  }

  if (typeof(subscriber) !== 'function') {
    return callback(new DeviceError('SUBSCRIBER_TYPE_ERROR'), null);
  }

  if (options.enableAPICache !== true || action.apiCache == null) {
    return callback(new DeviceError('EVENT_SUBSCRIPTION_FAIL'), null);
  }
  ...
  this.validateActionCall(action, input, true, function(err, data) {
    ...
    // here we assume the key already exists, which is created by previous action call which accessed the same key
    if (this.apiKeyStats[inputKey] != null && this.apiKeyStats[inputKey].keyObject != null) {
      this.apiKeyStats[inputKey].keyObject.addEventSubscriber(subscriber);
      return callback(null, this.apiKeyStats[inputKey].keyObject);
    }
    return callback(new DeviceError('EVENT_SUBSCRIPTION_FAIL'), null);
  }.bind(this));
...
Service.prototype.unsubscribeEvent = function(subscriber, actionName, input, inputKey, callback) {
  var action = this.actions[actionName];
  ...
  this.validateActionCall(action, input, true, function(err, data) {
    ...
    // here we assume the key already exists, which is created by previous action call which accessed the same key
    if (this.apiKeyStats[inputKey] != null && this.apiKeyStats[inputKey].keyObject != null) {
      this.apiKeyStats[inputKey].keyObject.removeEventSubscriber(subscriber);
      return callback(null, this.apiKeyStats[inputKey].keyObject);
    }
    return callback(new DeviceError('EVENT_SUBSCRIPTION_FAIL'), null);
  }.bind(this));
