...
Service.prototype.validateActionCall = function(action, arguments, isInput, callback) {
  ...
  if (arguments == null) {
    return callback(new CdifError('ARGUMENTS_INVALID'));
  }
  ...
  if (isInput) {
    for (var i in argList) {
      if (argList[i].direction === 'in') {
        if (arguments[i] === undefined) {
          failed = true;
          error = new CdifError('MISSING_INPUT_ARGUMENT', i);
          break;
        }
      }
    }
    ...
    for (var i in argList) {
      if (argList[i].direction === 'out') {
        if (arguments[i] === undefined) {
          failed = true;
          error = new CdifError('MISSING_OUTPUT_ARGUMENT', i);
          break;
        }
      }
    }
  ...
  for (var i in arguments) {
    ...
    if (isInput && argList[i].direction === 'out') {
      ...
      validator.validate(name, stateVar, arguments[i], function(err) {
        if (err) {
          error = new CdifError('DATA_VALIDATION_FAIL', err.message);
          failed = true;
        }
      });
    }
    ...
    action.invoke(input, function(err, output) {
      if (err) {
        ...
        if (output && output.fault) {
          return session.callback(new DeviceError('DEVICE_INVOKE_FAIL', err.message), output.fault);
        }
        return session.callback(new DeviceError('DEVICE_INVOKE_FAIL', err.message), null);
      }
