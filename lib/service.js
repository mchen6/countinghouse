        ...
        this.device.resolveSchemaFromPath(action.fault.schema, self, function(err, s, data) {
          if (!err) {
            try {
              ...
              s.validator = null;
              LOG.DE(this.device, new McpForgeError('SCHEMA_COMPILE_ERROR', e.message));
            }
...
Service.prototype.setServiceStates = function(values, callback) {
  ...
  if (errorMessage) {
    callback(new McpForgeError('SET_SERVICE_STATE_ERROR', errorMessage));
  } else {
...
Service.prototype.updateStateFromAction = function(action, args, output, callback) {
  if (options.apiCache === true && action.apiCache != null) {
    ...
    redisAPI.client.hmget(hashString, 'deviceID', function(err, results) {
      if (err) {
        LOG.E(new McpForgeError('REDIS_CLIENT_ERROR', err.message));
        return callback(false);
      }
...
Service.prototype.invokeAction = function(actionName, args, session) {
  ...
  if (action == null) {
    return cb(new McpForgeError('ACTION_NOT_FOUND', actionName), null);
  }
  if (args == null) {
    return cb(new McpForgeError('INPUT_NOT_FOUND'), null);
  }
    ...
    if (typeof(session) !== 'function') {
      if (action.realPrice > 0) {
        ...
        if (session.apiRemainCount <= 0 && session.balance < action.realPrice) {
          return session.callback(new McpForgeError('NOT_ENOUGH_USER_BALANCE', actionName), null);
        }
      }
...
Service.prototype.doActionCall = function(action, args, session) {
  ...
  unsafeDomain.on('error', function(err) {
    return cb(new DeviceError('DEVICE_INVOKE_EXCEPTION'), err);
  });
  ...
  unsafeDomain.run(function() {
    ...
    if (isAsync === true) {
      (async function() {
        let output = await action.invoke(args);

        async function validateOutputAndUpdateAPICache() {
        ...
        await validateOutputAndUpdateAPICache();
...
Service.prototype.subscribeEvent = function(actionName, input, inputKey, callback) {
  ...
  if (action == null) {
    return callback(new McpForgeError('ACTION_NOT_FOUND', actionName));
  }
  if (input == null) {
    return callback(new McpForgeError('INPUT_NOT_FOUND'));
  }
  if (action.invoke == null) {
