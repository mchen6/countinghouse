    ...
    redisClient.hmget(hashString, 'deviceID', 'valueHash', function(err, results) {
      ...
      if (results[0] == null) {
        ...
        redisClient.hmset(hashString, 'deviceID', this.device.deviceID, 'value', outputString, 'valueHash', outputHashString, 'timestamp', Date.now(), function(err) {
          this.apiKeyStats[hashString] = false;
          if (err) {
            LOG.E(new CdifError('REDIS_CLIENT_ERROR', err.message));
            return callback(false);
          }
...
Service.prototype.getValueFromAPICache = function(actionName, timeout, input, callback) {
  ...
  if (this.apiKeyStats[hashString] == null) {
    this.apiKeyStats[hashString] = false; // initial refreshing state set to false
  }

  //af: key access frequency, at: key access time
  redisClient.hmget(hashString, 'deviceID', 'value', 'timestamp', 'af', 'at', function(err, results) {
    if (err) {
      LOG.E(new CdifError('REDIS_CLIENT_ERROR', err.message));
    ...
    if (now - cacheTimeStamp > timeout) {
      if (this.apiKeyStats[hashString] === false) {
        ...
        this.apiKeyStats[hashString] = true;
