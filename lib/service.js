    ...
    redisClient.hmget(hashString, 'deviceID', 'valueHash', function(err, results) {
      ...
      if (results[0] == null) {
        ...
        redisClient.hmset(hashString, 'deviceID', deviceID, 'value', outputString, 'valueHash', outputHashString, 'timestamp', Date.now(), function(err) {
          this.apiRefreshingKeys[hashString] = false;
          if (err) {
            LOG.E(new CdifError('REDIS_CLIENT_ERROR', err.message));
            return callback(false);
          }
...
Service.prototype.getValueFromAPICache = function(actionName, timeout, input, callback) {
  ...
  // if (this.apiKeyStats[hashCode.toString()] == null) {
  //   this.apiKeyStats[hashCode.toString()] = {};
  // }
  // apiKeyStats[hashCode.toString()]
  ...
  redisClient.hmget(hashString, 'deviceID', 'value', 'timestamp', function(err, results) {
    ...
    if (Date.now() - cacheTimeStamp > timeout) {
      if (this.apiRefreshingKeys[hashString] == null || this.apiRefreshingKeys[hashString] === false) {
        LOG.I('value expired for input key, do slow action call to get new value: ' + inputKey);
        this.apiRefreshingKeys[hashString] = true;
        return callback(null, null);
      }
    }
