...
Service.prototype.doActionCall = function(action, input, session) {
  ...
  unsafeDomain.on('error', function(err) {
    return session.callback(new DeviceError('DEVICE_INVOKE_EXCEPTION', {message: err.message, stackInfo: err.stack}), null);
  });
    ...
    action.invoke(args, function(err, output) {
      if (err) {
        ...
        if (err instanceof DeviceError || err instanceof CdifError) {
          ...
          error = new DeviceError('DEVICE_INVOKE_FAIL', {message: err.message, stackInfo: err.stack});
        }
