...
Service.prototype.invokeAction = function(actionName, input, session) {
  ...
  if (options.enableAPIMonitor === true) {
    // the web site code would make sure only priced actions will have priceInfo and store it in user's device list
    if (session.priceInfo != null && session.priceInfo[this.serviceID] != null && session.priceInfo[this.serviceID][actionName] != null) {
      if (session.priceInfo[this.serviceID][actionName].freeCount <= 0) {
