...
var ARGUMENT_KEYS = {input: true, output: true};
...
var FRAMEWORK_ARG_KEYS = {ctx: true, httpHeaders: true, jobID: true};
...
Service.prototype.resolveSchemaObject = function(obj) {
  this.device.resolveSchemaFromPath(obj.schema, obj, function(err, s, data) {
...
Service.prototype.validateActionCall = function(action, args, isInput, callback) {
  var key       = (isInput === true) ? 'input' : 'output';
  var errorCode = (isInput === true) ? 'INPUT_DATA_VALIDATION_FAIL' : 'OUTPUT_DATA_VALIDATION_FAIL';
  ...
  validator.validate(key, schemaObj, args[key], function(errMsg, errInfo) {
    if (errMsg) {
