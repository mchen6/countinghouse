  ...
  if (options.enableAPICache === true && action.apiCache != null) {
    ...
    redisClient.hmget(hashString, 'deviceID', 'valueHash', function(err, results) {
      ...
      if (results[0] == null) {
        ...
        redisClient.hmset(hashString, 'deviceID', deviceID, 'value', outputString, 'valueHash', outputHashString, 'timestamp', Date.now(), function(err) {
  ...
  for (var i in arguments) {
    ...
    if (isInput && argList[i].direction === 'out') {
      ...
      validator.validate(i, stateVar, arguments[i], function(errMsg, errInfo) {
        if (errMsg) {
          if (isInput === true) {
            error = new CdifError('INPUT_DATA_VALIDATION_FAIL');
          } else {
            error = new CdifError('OUTPUT_DATA_VALIDATION_FAIL');
          }
  ...
  this.validateActionCall(action, input, true, function(err, data) {
    ...
    if (options.enableAPICache === true && action.apiCache != null) {
      this.getValueFromAPICache(actionName, action.apiCache, input, function(err, value, freq) {
        ...
        // if it access the key within this 100 seconds period. To save push bandwidth, we push no more than
        // one message in 3 seconds and push only 3 times (which could be longer than 9 seconds),
        // further push requires renew subscription to the key, which will happen when user access the key again
...
Service.prototype.getValueFromAPICache = function(actionName, timeout, input, callback) {
  ...
  redisClient.hmget(hashString, 'deviceID', 'value', 'timestamp', function(err, results) {
    ...
    if (results[0] !== deviceID) return callback(null, null, null);

    var freq = this.getApiInputKeyAccessInfo(hashCode.toString());
