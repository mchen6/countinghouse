...
// we need to add below code in doActionCall if we want to support async function in user's code which should return a promise to us
...
//     return cb(new DeviceError('DEVICE_INVOKE_FAIL', err.message));
//   });
//
...
//     CdifUtil.redis.get('BBB', function(err, data) {
//       try {
...
Service.prototype.doActionCall = function(action, args, session) {
  ...
  if (isAsync === true) {
    (async function() {
      ...
      function validateOutputAndUpdateAPICache() {
        return new Promise((resolve, reject) => {
      ...
      //LIMITATION: in case of an exception, we can't carry back output object along with the err object
      return cb(new DeviceError('DEVICE_INVOKE_EXCEPTION', err.message));
    });
  } else {
    var unsafeDomain = Domain.create();
    unsafeDomain.on('error', function(err) {
      // console.log('BBBBBBB: ' + err.stack);
