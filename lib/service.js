          ...
          this.device.resolveSchemaFromPath(schemaRef, self, function(err, s, data) {
            if (!err) {
              try {
                ...
                s.validator = null;
                LOG.DE(this.device, new CdifError('compile schema error: ' + e.message));
              }
...
Service.prototype.updateStateFromAction = function(action, input, output, callback) {
  ...
  if (options.enableAPICache === true && action.apiCache != null) {
    ...
    redisClient.hmget(hashCode, 'deviceID', 'valueHash', function(err, results) {
      if (err) return callback(false, output);
      if ((results[0] != null && results[0] !== deviceID)) {
        LOG.I('hash collision for input key: ' + inputKey);
        return callback(false, output); // hash collision
      }

      if (results[0] == null) {
        LOG.I('set value for input key: ' + inputKey);
        redisClient.hmset(hashCode, 'deviceID', deviceID, 'value', outputString, 'valueHash', outputHashCode, 'timestamp', Date.now());
        updated = true;
      } else {
        // deliberately use string to number compare, it seems everything returned from redis are strings
        if(results[1] != outputHashCode) {
          redisClient.hmset(hashCode, 'value', outputString, 'valueHash', outputHashCode, 'timestamp', Date.now());
          updated = true;
        } else {
          redisClient.hmset(hashCode, 'timestamp', Date.now());
        }
      }
      if (updated === true) LOG.I('api cache updated for input key: ' + inputKey);
      return callback(updated, output);
    });
  }
...
Service.prototype.invokeAction = function(actionName, input, session) {
  ...
  if (action === undefined) {
    return session.callback(new CdifError('action not found: ' + actionName), null);
  }
  if (input === undefined) {
    return session.callback(new CdifError('cannot identify input arguments'), null);
  }
  if (action.invoke === null) {
    return session.callback(new DeviceError('action: ' + actionName + ' not implemented'), null);
  }
    ...
    if (action.realPrice > 0) {
         ...
         )
      {
        if (session.balance < action.realPrice) {
          return session.callback(new CdifError('not enough user balance: ' + actionName), null);
        }
      }
    }
...
Service.prototype.getValueFromAPICache = function(actionName, timeout, input, callback) {
  ...
  redisClient.hmget(hashCode, 'deviceID', 'value', 'timestamp', function(err, results) {
    ...
    if (Date.now() - cacheTimeStamp > timeout) {
      LOG.I('value expired for input key, getting new value: ' + inputKey);
...
Service.prototype.doActionCall = function(action, input, session) {
  ...
  unsafeDomain.on('error', function(err) {
    return session.callback(new DeviceError(err.message), null);
  });
    ...
    action.invoke(input, function(err, output) {
      if (err) {
        ...
        if (output && output.fault) {
          return session.callback(new DeviceError(err.message), output.fault);
        }
        return session.callback(new DeviceError(err.message), null);
      }
      this.validateActionCall(action, output, false, function(error) {
        if (error) {
          return session.callback(error, null);
        }
        this.updateStateFromAction(action, input, output, function(updated, data) {
            this.emit('serviceevent', updated, this.device.deviceID, this.serviceID, data);
        }.bind(this));
