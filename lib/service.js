...
Service.prototype.invokeAction = function(actionName, input, session) {
  ...
  var fileName = this.actions[actionName].fileName;
    ...
    if (options.enableAPICache === true && action.apiCache != null) {
      this.getValueFromAPICache(actionName, action.apiCache, input, function(err, value, freq) {
        ...
        if (err != null || value == null) {
          return this.doActionCall(action, fileName, input, session);
        }
...
Service.prototype.doActionCall = function(action, fileName, input, session) {
  console.log(fileName);

  if (isMainThread) {
    var worker  = new Worker('/home/mchen6/cdif.code/build/sandbox.js');
