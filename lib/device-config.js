...
// `byModuleName` -- see docs/design-decisions.md's
...
function loadDeviceConfigFromFile(opts, moduleName, callback) {
  const configDir  = opts.deviceConfigPath || (`${process.cwd()}/config/devices`);
  const configPath = path.join(configDir, `${moduleName}.json`);
