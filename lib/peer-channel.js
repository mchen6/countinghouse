...
PeerChannel.prototype.invoke = function(deviceID, serviceID, actionName, input, timeoutMs, callback, billingKey) {
  ...
  const timer = setTimeout(() => {
    ...
    _this.sendQueue = _this.sendQueue.filter((item) => { return item.id !== id; });
    return callback(new DeviceError('PEER_CHANNEL_TIMEOUT'));
  }, timeoutMs);

  this.pending[id] = {callback: callback, timer: timer};

  // billingKey travels per call, not per port. The grant-time appKey cannot
  // serve as the billing identity here: D1 keys a port by *worker pair*, so
  // one port carries calls made on behalf of many different outer callers.
  // On the request only -- never on the reply, for the reason the
  // _dispatchInvoke comment below spells out.
  const request = {id: id, deviceID: deviceID, serviceID: serviceID, actionName: actionName, input: input, billingKey: billingKey};
...
PeerChannel.prototype._handleInvoke = function(msg) {
  if (typeof(this.onInvoke) !== 'function') {
    const noHandlerErr = new DeviceError('PEER_NO_HANDLER');
...
PeerChannel.prototype._dispatchInvoke = function(msg) {
  ...
  // platformMetering (3rd callback arg, D5) rides alongside `data` rather
  // than being merged into it -- `data` is the callee action's own,
  // already-schema-validated return value (see
  // Service.prototype.validateActionCall), and a module on the caller
  // side may pass that value straight through as *its own* action's
  // output (e.g. echo-device-client-module's 服务名称-API名称.js). Merging
  // an extra field into it would make that pass-through fail validation
  // on the caller's side with an unrelated "OUTPUT_DATA_VALIDATION_FAIL"
  // (found the hard way: validateActionCall throws synchronously walking
  // an unexpected key, and that throw crosses a port-message boundary
  // where lib/service.js's Domain-based catch doesn't reliably reach,
  // silently hanging the call until the session's own 30s timeout).
  this.onInvoke(msg, (err, data, platformMetering) => {
    ...
    _this.port.postMessage({
      ...
      data: data,
      platformMetering: platformMetering
    });
...
PeerChannel.prototype._handleReply = function(msg) {
  ...
  const entry = this.pending[msg.id];
  ...
  if (msg.errMsg != null) {
    const err = new Error(msg.errMsg);
    if (msg.errCode != null) err.code = msg.errCode;
    return entry.callback(err, msg.data);
  }
  return entry.callback(null, msg.data, msg.platformMetering);
};
