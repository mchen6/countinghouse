...
function RouteManager(mm) {
 ...
 // this.app.use(bodyParser.urlencoded({extended: true}));
  ...
  //per device routes
  //user validation
  this.deviceControlRouter.use('/:deviceID', require('./routes/sms-template-verify-callback'));
...
RouteManager.prototype.installRoutes = function() {
  ...
  if (options.enableVerifyAndPublish === true) {
    this.moduleVerifyRouter.route('/').post(function (req, res) {
      ...
      this.moduleManager.verifyModule(packageName, session.callbackWithoutTimer.bind(session));
    }.bind(this));
  ...
  this.actionInvokeRouter.route('/').post(function(req, res) {
    ...
    this.cdifInterface.invokeDeviceAction(deviceID, serviceID, actionName, argumentList, token, session);
  }.bind(this));
