...
var CdifInterface = require('./mcpforge-interface');
...
RouteManager.prototype.mountDevicePresentationPage = function(deviceID) {
  ...
  session.callback = function(err, deviceUrl) {
    if (!err) {
      ...
      LOG.E(new McpForgeError('GET_DEVICE_ROOTURL_FAIL', err.message));
    }
  }.bind(this);
...
RouteManager.prototype.startServer = function() {
  ...
  this.server.listen(options.port, McpForgeUtil.getHostIp());
  LOG.I('mcpforge listen on: ' + McpForgeUtil.getHostIp() + ':' + options.port);
};
