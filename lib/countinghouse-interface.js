...
function CdifInterface(mm) {
  ...
  // per-apiKey rate limit, additional to (and independent of) the combined-
  // callers globalRateLimit above. Opt-in via --apiKeyRateLimit; off by
  // default, so this never changes behavior for a deployment/test run that
  // doesn't set it.
  if (options.apiKeyRateLimit != null && isMainThread === true) {
...
CdifInterface.prototype.invokeDeviceAction = function(deviceID, serviceID, actionName, args, token, session) {
  ...
  var checkApiKeyRateLimit = function() {
    if (_this.meteringProvider == null || session.appKey == null) return doInvoke();

    _this.meteringProvider.rateLimit(session.appKey, function(err, result) {
