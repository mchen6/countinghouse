...
Session.prototype.logAPICall = function(err, input, data, isHTTPCall) {
  ...
  if (options.enableAPIMonitor === true) {
    ...
    //for invalid input the error is published to redis and subscribed / received by client at real time
    if (this.serviceID != null && this.actionName != null) {
      ...
      if (sl == null) return;
      if (sl[this.serviceID] == null) return;
      if (sl[this.serviceID].actionList == null) return;
      if (sl[this.serviceID].actionList[this.actionName] == null) return;
