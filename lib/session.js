...
if (isMainThread === true) {
  ...
  // db 0, matching the plain redis.createClient(options.redisUrl) above and
  // lib/user-auth.js's own client -- appKey-keyed balance/devices/
  // toolPriceRecord records all need to live in the same logical db.
  meteringProvider = new RedisMeteringProvider({redisClient: redisClient});
}
      ...
      if (this.device instanceof WorkerMessage) {
        ...
        for (var deviceID in deviceList) {
          if (deviceID === this.deviceID) {
            var sl = deviceList[deviceID].device.serviceList;
...
//
// Delegates to the RedisMeteringProvider (lib/metering/redis-provider.js)
// rather than deducting inline. `count` (always 1 from the one real call
// site below) is folded into `cost` up front since MeteringProvider.
// recordCall's signature is (apiKey, tool, cost, callback), not a separate
// per-call count. The `tool` identifier is built with encodeLegacyTool so
// the provider's read path still honors existing free-call counts already
// stored under the old devices[].priceRecord[serviceID][actionName] shape
// (see redis-provider.js's recordCall for the adapter logic); new
// deductions are always written to the provider's own toolPriceRecord
// field going forward, not back into the old `devices` blob.
Session.prototype.updateRedisUserRecord = function(appKey, deviceID, serviceID, actionName, count) {
  // we do redis API log only in main thread
  if (isMainThread !== true || redisClient == null || meteringProvider == null) return;
  //update only priced api, no need to update free api
  if (this.realPrice > 0 && this.userDevices != null) {
    var tool = RedisMeteringProvider.encodeLegacyTool(deviceID, serviceID, actionName);
    var cost = this.realPrice * count;

    meteringProvider.recordCall(appKey, tool, cost, function(err, result) {
