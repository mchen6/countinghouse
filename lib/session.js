...
function Session(req, res, username, appKey, balance, deviceID, apiRemainCount, localCallback) {
  ...
  this.localCallback  = localCallback;      // a local callback from other services in the same mcpforge instance, only available in service-client
...
Session.prototype.logAPICall = function(err, input, data, isHTTPCall) {
  ...
  if (err != null) {
    if (options.debug === true) {
      if (input != null) {
        LOG.E(new McpForgeError('DEVICE_ACCESS_ERROR', err.message, 'input:', input, 'fault: ', data));
      } else {
        LOG.E(new McpForgeError('DEVICE_ACCESS_ERROR', err.message, 'fault: ', data));
      }
    }
  }
