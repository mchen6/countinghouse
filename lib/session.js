...
Session.prototype.response = function(err, data) {
  if (err) {
    ...
    if (options.enableAPIMonitor === true) {
      // the serviceID and actionName fields are set by service code when action call is successfully completed
      if (pub != null && this.serviceID != null && this.actionName != null) {
        ...
        if (this.apiLog === true && this.req != null && this.req.body != null && this.req.body.input != null && data != null) {
          message = JSON.stringify({appKey: this.appKey, username: this.username, timestamp: Date.now(), freeAPICount: freeAPICount, input: this.req.body.input, output: data.output});
        } else {
          message = JSON.stringify({appKey: this.appKey, username: this.username, timestamp: Date.now(), freeAPICount: freeAPICount});
        }
