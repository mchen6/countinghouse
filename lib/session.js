...
Session.prototype.response = function(err, data) {
  if (this.req != null && this.req.body != null) {
    this.logAPICall(err, this.req.body, data);
  } else {
    this.logAPICall(err, null, data);
  }
...
Session.prototype.logAPICall = function(err, input, data) {
  if (err != null) {
    if (input != null) {
      return LOG.DE(this.device, new CdifError('DEVICE_ACCESS_ERROR', err.message, 'input:', input, 'fault:', data));
    } else {
      return LOG.DE(this.device, new CdifError('DEVICE_ACCESS_ERROR', err.message, 'fault:', data));
    }
  }
