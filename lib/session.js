...
function Session(req, res, username, appKey, balance, deviceID, priceInfo) {
  this.req       = req;
  this.res       = res;
  this.username  = username;  // user's name
  this.appKey    = appKey;    // user's appKey
  this.balance   = balance;   // user's balance
  this.priceInfo = priceInfo; //only priced action would have non-null priceInfo
  ...
  if (err) {
    ...
    if (options.enableAPIMonitor === true) {
      if (pub != null) {
        ...
        var message = JSON.stringify({username: this.username, appKey: this.appKey});
