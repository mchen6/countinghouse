...
Session.prototype.callback = function(err, data) {
  ...
  if (this.timer.expired === true) {
    ...
    if (this.localCallback != null) {
      this.logAPICall(err, this.localInput, data);
      return this.localCallback(new Error(err.message), null);
    }
    this.logAPICall(err, this.req.body, data);
...
//
Session.prototype.logAPICall = function(err, input, data) {
  if (err != null) {
    if (input != null) {
  ...
  if (options.enableAPIMonitor === true) {
    var now = Date.now();
    if (err != null) {
      var errorChannel = this.appKey;
      redisClient.multi()
      .sadd('errornightlyset', errorChannel)
      .rpush('errorlist:' + errorChannel, now)
      .rpush('errorinfo:' + errorChannel, JSON.stringify({message: err.message, deviceID: this.deviceID, serviceID: this.serviceID, actionName: this.actionName, input: input, fault: data }))
