...
function Session(req, res, username, appKey, balance, deviceID, apiRemainCount, localCallback) {
  ...
  this.localCallback  = localCallback;      // a local callback from other services in the same cdif instance
...
Session.prototype.callback = function(err, data) {
  ...
  if (this.timer.expired === true) {
    this.timer = null;
    if (this.localCallback == null) return this.res.status(500).json({topic: err.topic, message: err.message});
  }
...
Session.prototype.response = function(err, data) {
  if (err) {
    ...
    if (this.req.body != null && this.req.body.input != null) {
      return LOG.DE(this.device, new CdifError('DEVICE_ACCESS_ERROR', err.message, 'input:', this.req.body.input, 'fault:', data));
    } else {
