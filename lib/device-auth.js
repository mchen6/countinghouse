...
DeviceAuth.prototype.verifyAccess = function(secret, token, callback) {
  if (typeof(token) !== 'string') {
    return callback(new McpForgeError('INVALID_ACCESS_TOKEN'));
  }
  if (typeof(secret) !== 'string') {
    return callback(new McpForgeError('CANNOT_VERIFY_ACCESS_TOKEN'));
  }
  ...
  try {
    ...
    return callback(new McpForgeError('CANNOT_VERIFY_ACCESS_TOKEN', e.message));
  }
  callback(null);
};
...
DeviceAuth.prototype.getSecret = function(deviceID, pass, callback) {
  ...
  //   if (err) {
  //     return callback(new McpForgeError(err.message), null);
  //   }
  //   if (!data) {
  //     bcrypt.hash(pass, 8, function(e, secret) {
  //       if (e) {
  //         callback(new McpForgeError(e.message), null);
  //       } else {
  //         deviceDB.storeSecret(deviceID, secret, function(error) {
  //           if (error) {
  //             callback(new McpForgeError(error.message), null);
  //           } else {
  ...
  //       if (err) {
  //         callback(new McpForgeError(err.message), null);
