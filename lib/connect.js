...
ConnectManager.prototype.verifyConnect = function(user, pass, callback) {
  ...
  //     return callback(new CHError('cannot verify password'), null, null);
  //   }
  //   if (this.device.user !== user) {
  //     return callback(new CHError('username not match'), null, null);
  //   }
  ...
  //     }
  //     if (!err) {
  //       return callback(new CHError('password not match'), null, null);
  //     }
  //     return callback(new CHError(err.message), null, null);
  //   }.bind(this));
  // } else {
  //   callback(null, null, null);
  // }
...
ConnectManager.prototype.processConnect = function(user, pass, callback) {
  if (this.device._connect && typeof(this.device._connect) === 'function') {
    this.device._connect(user, pass, (error, redirectObj) => {
      if (error) {
  ...
  //     callback(new CHError('cannot authenticate this device'), null, null);
  //   } else {
  //     this.device.connectionState = 'connected';
  //     callback(null, null, null);
  //   }
