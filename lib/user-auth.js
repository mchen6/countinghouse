...
var doUserAuth = function(req, res, deviceID, appKey, serviceID, actionName, localCB, callback) {
  if (redisClient == null && isMainThread === true) {
  ...
  if (redisClient == null) return callback(new McpForgeError('REDIS_CLIENT_ERROR'), 'redis client not available');
  ...
  if (options.debug === true) {
    ...
    if (appKey == null || appKey !== debugKey) {
      return callback(new McpForgeError('SYSTEM_ERROR_UNKNOWN_USER'));
    }
    var session = new Session(req, res, 'debug', appKey, 0, deviceID, 0, localCB);
    session.serviceID  = serviceID;
    session.actionName = actionName;
  ...
  if (usersDB == null) {
    return callback(new McpForgeError('SYSTEM_ERROR_USERDB_NA'));
  }

  redisClient.hmget(appKey, 'userName', 'balance', 'devices', function(err, results) {
    if (err) {
      return callback(new McpForgeError('REDIS_CLIENT_ERROR'));
    }
    if (results[0] == null) { // key not exists in redis, turn to couch
      usersDB.view('user', 'byAppKeyWithBasicInfo', { key: appKey }, function(err, doc) {
        if (err) {
          return callback(new McpForgeError('SYSTEM_ERROR_USERDB_NA', err.message));
        }
        if (doc.rows.length > 0) {
          ...
          if (hasDevice === false)          return callback(new McpForgeError('USER_HAS_NO_DEVICE'));
          if (typeof(balance) !== 'number') return callback(new McpForgeError('INVALID_BALANCE_FORMAT')); // balance format is invalid in user's record
          if (priceRecord == null)          return callback(new McpForgeError('NO_PRICE_RECORD')); //there is no priceRecord field in user's device list
        ...
        return callback(new McpForgeError('SYSTEM_ERROR_UNKNOWN_USER', appKey));
      });
    } else {
      ...
      if (results[2] != null) {
        ...
        try {
          ...
          return callback(new McpForgeError('USER_DEVICE_RECORD_FORMAT_INVALID', e.message));
        }
