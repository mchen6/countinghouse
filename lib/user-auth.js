...
var doUserAuth = function(req, res, deviceID, appKey, serviceID, actionName, localCB, callback) {
  ...
  if (redisClient == null) return callback(new CHError('REDIS_CLIENT_ERROR'), 'redis client not available');
  ...
  if (usersDB == null) {
    return callback(new CHError('SYSTEM_ERROR_USERDB_NA'));
  }

  redisClient.hmget(appKey, 'userName', 'balance', 'devices', function(err, results) {
    if (err) {
      return callback(new CHError('REDIS_CLIENT_ERROR'));
    }
    if (results[0] == null) { // key not exists in redis, turn to couch
      usersDB.view('user', 'byAppKeyWithBasicInfo', { key: appKey }, function(err, doc) {
        if (err) {
          return callback(new CHError('SYSTEM_ERROR_USERDB_NA', err.message));
        }
        if (doc.rows.length > 0) {
          ...
          if (hasDevice === false)          return callback(new CHError('USER_HAS_NO_DEVICE'));
          if (typeof(balance) !== 'number') return callback(new CHError('INVALID_BALANCE_FORMAT')); // balance format is invalid in user's record
          if (priceRecord == null)          return callback(new CHError('NO_PRICE_RECORD')); //there is no priceRecord field in user's device list
        ...
        return callback(new CHError('SYSTEM_ERROR_UNKNOWN_USER', appKey));
      });
    } else {
      ...
      if (results[2] != null) {
        ...
        try {
          ...
          return callback(new CHError('USER_DEVICE_RECORD_FORMAT_INVALID', e.message));
        }
