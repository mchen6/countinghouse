...
ModuleManager.prototype.onModuleLoad = function(name, module, version) {
  ...
  if (options.allowDiscover === true && options.workerThread === true) {
    //TODO: send discover event to worker
  ...
  if (options.allowDiscover === false) {
    if (options.workerThread === true && isMainThread === true) {
      ...
      LOG.I('Loaded modules: ' + _mm.noofLoadedModules);
      LOG.I('Total modules: ' + _mm.noofTotalModules);
    ...
    } else if (options.workerThread === false && isMainThread === false) {
      //worker thread mode but run in child thread
  ...
  deviceDB.getAllModuleInfo(function(err, data) {
    ...
    data.forEach(function(item) {
      this.loadModuleFromPath(item.path, item.name, item.version, function(err, mi) {
        if (err) return;
        //only need to update counter here under non-thread mode
        //this method won't be executed by child thread
        this.noofTotalModules ++;
        LOG.I('XXX: ' + this.noofTotalModules);
      }.bind(this));
    }.bind(this));
  }.bind(this));
};
  ...
  if (wm != null) {
    wm.sendLoadModuleMessage({path: path, name: name, version: version}, function(e, d) {
      if (e) {
        ...
        var errMsg = 'load module failed: ' + name + '@' + version + ', error info: ' + e.message;
        LOG.E(errMsg);
    ...
    workerMessage.sendSetOptionsMessage({options: options.getOptions()}, function(err, data) {
      ...
      workerMessage.sendLoadModuleMessage({path: path, name: name, version: version}, function(e, d) {
        ...
        //workerMessage acting as moduleInstance in main thread
