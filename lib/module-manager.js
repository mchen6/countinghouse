...
//NOTICE: this API is no longer used by cdif, all independent module load would use loadModuleFromPath call
            ...
            unsafeDomain.run(function() {
              _this.loadModuleFromPath(path, name, version, function(e, mi) {
                ...
                chmodr(installBasePath, 0777, function(err) {
                  ...
                  fs.writeFileSync(path + '/package.json', JSON.stringify(packageInfo, null, 2), 'utf-8');

                  if (options.cloud9 === true) {
                    C9Launcher.startCloud9(path, CdifUtil.getHostIp(), options.dbUrl, function(errr) {
