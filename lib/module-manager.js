...
ModuleManager.prototype.verifyModule = function(path, callback) {
  ...
  // record device info, unload module, uninstall package
};

ModuleManager.prototype.onQueryDeviceListResult = function(error, deviceList, moduleName, moduleVersion, callback) {
  this.uninstallModule(moduleName, function(err) {
    if (err) {
      return callback(err, {name: moduleName, version: moduleVersion});
    }
    if (error) {
      return callback(error, {name: moduleName, version: moduleVersion});
    } else {
      return callback(null, {name: moduleName, version: moduleVersion, deviceList: deviceList});
    }
  });
};
