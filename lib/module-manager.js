...
var CHUtil       = require('../lib/countinghouse-util');
var options        = require('../lib/cli-options');
var exec           = require('child_process').exec;
var CHError      = require('../lib/countinghouse-error').CHError;
...
ModuleManager.prototype.verifyModule = function(input, callback) {
  ...
  var tar = unzip.pipe(new tarModule.Parser());
...
ModuleManager.prototype.loadModuleUnsafe = function(modulePath, name, version, packageInfo, apiDesignID, callback) {
  ...
  if (options.cloud9 === true) {
    C9Launcher.startCloud9(modulePath, CHUtil.getHostIp(), options.dbUrl, apiDesignID, function(errr) {
      if (errr) return callback(new CHError('MODULE_INSTALL_FAIL', errr.message));
    ...
    VSCodeLauncher.startVSCode(modulePath, apiDesignID, function(errr) {
      if (errr) return callback(new CHError('MODULE_INSTALL_FAIL', errr.message), {packageInfo: packageInfo, deviceList: [], moduleInstallPath: modulePath});
      unsafeDomain.run(function() { _this.loadModuleAndQueryDevice(modulePath, name, version, packageInfo, callback); });
    });
  } else {
    unsafeDomain.run(function() { _this.loadModuleAndQueryDevice(modulePath, name, version, packageInfo, callback); });
