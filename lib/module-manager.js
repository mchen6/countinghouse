...
ModuleManager.prototype.getModuleDeviceListByName = function(name, callback) {
  ...
  setTimeout(function() {
    ...
    _this.emit('querydevicelist', moduleInstance, {name: name}, function(err, info) {
      return callback(err, info);  //we are interested at info.deviceList
    });
  }, 1000);
}
  ...
  tar.on('entry', function(entry) {
    if (/package\.json$/.test(entry.path)) { // look into package.json
      ...
      bufferStream.on('data', function(chunk) {
        ...
        var installBasePath = path.join(input.path, name + '-' + version);
        ...
        try {
          exec(command, {timeout: 120000}, function(err, stdout, stderr) {
            ...
            var modulePath = installBasePath + '/node_modules/' + name;
              ...
              _this.loadModuleFromPath(modulePath, name, version, function(e, mi) {
                ...
                chmodr(installBasePath, 0666, function(err) {
                  ...
                  if (options.cloud9 === true) {
                    C9Launcher.startCloud9(modulePath, CdifUtil.getHostIp(), options.dbUrl, input.apiDesignID, function(errr) {
                      if (errr) return callback(new CdifError('MODULE_INSTALL_FAIL', errr.message));
                    });
                  } else if (options.vscode === true) {
                    VSCodeLauncher.startVSCode(modulePath, input.apiDesignID, function(errr) {
                      if (errr) return callback(new CdifError('MODULE_INSTALL_FAIL', errr.message));
                    });
                  }
