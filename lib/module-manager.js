...
ModuleManager.prototype.loadModuleByWorker = function(path, name, version, callback) {
  if (isMainThread) {
    ...
    workerMessage.sendSetOptionsMessage(worker, {options: options.getOptions()}, function(err, data) {
      if (err) {
        worker.terminate();
        return callback(err, data);
      }

      workerMessage.sendLoadModuleMessage(worker, {path: path, name: name, version: version}, function(e, d) {
        if (e) worker.terminate();
        return callback(e, d);
      });
    });
  }
