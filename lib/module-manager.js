...
ModuleManager.prototype.onDeviceOnline = function(packageInfo, device, module) {
  var found = false;
  for (var moduleName in this.modules) {
    if (this.modules[moduleName] === module) {
      found = true;
      this.emit('deviceonline', device, module, moduleName, packageInfo);
    }
  }
  if (found === false) {
...
ModuleManager.prototype.loadModuleFromPath = function(modulePath, name, version, callback) {
  ...
  moduleInstance.on('deviceonline',  this.onDeviceOnline.bind(this, packageInfo));  //append content of package.json to module instance
...
ModuleManager.prototype.unloadModuleExternal = function(name, callback) {
  ...
  if (m != null) {
    if (m instanceof WorkerMessage) {
      ...
      // if device is blocked by user code, terminate thread and return after 3 seconds
      setTimeout(function() {
        if (this.modules[name] != null) {
          this.unloadModule(name, function() {
            m.worker.terminate();
...
ModuleManager.prototype.unloadModule = function(name, callback) {
  ...
  if (m != null) return this.emit('moduleunload', name, callback);
...
ModuleManager.prototype.verifyModule = function(input, callback) {
  ...
  tar.on('end', function() {
    if (installBasePath == null || modulePath == null) return callback(new CdifError('MODULE_INSTALL_FAIL', 'unknown install path'));
    if (packageInfo == null) return callback(new CdifError('MODULE_INSTALL_FAIL', 'unknown package info'));
    ...
    // do fresh npm install from here
    var command = 'npm install ' + '--prefix ' + installBasePath + ' --registry=' + registry + ' ' + input.name; // install the zip package which is created by `npm pack` on website

    //FIXME: in case of a fresh npm install, we need to rimraf installBasePath so that .git folders will be removed before fresh install, or inconsistent behaviour would happen
    try {
      fs.accessSync(input.path, fs.W_OK);
      //do rimraf before npm install to ensure the install location clean to prevent EISGIT issue
      rimraf(installBasePath, function(err) {
        if (err) return callback(new CdifError('MODULE_INSTALL_FAIL', name, err.message));
        ...
        exec(command, {timeout: 120000}, function(err, stdout, stderr) {
          if (err) {
            ...
            return rimraf(installBasePath, function() {
          ...
          try {
            fs.writeFileSync(path.join(modulePath, 'package.json'), JSON.stringify(packageInfo, null, 2), {encoding: 'utf8', mode: 0666});
          } catch (e) {
            return callback(new CdifError('MODULE_INSTALL_FAIL', e.message), {packageInfo: packageInfo, deviceList: [], moduleInstallPath: modulePath});
          }
  ...
  tar.on('entry', function(entry) {
    if (/api\.json$/.test(entry.path)) {  //look into api.json
      ...
      bufferStream.on('data', function(chunk) {
        ...
        try {
          apiJson = JSON.parse(data.toString());
        } catch (e) {
    ...
    } else if (/schema\.json$/.test(entry.path)) { // look into schema.json
      if (entry.type === 'Directory') return;
      ...
      bufferStream.on('data', function(chunk) {
        ...
        try {
          schemaJson = JSON.parse(data.toString());
        } catch (e) {
          return callback(new CdifError('MODULE_PACKAGE_INFO_INVALID', e.message));
        }
    ...
    } else if (/package\.json$/.test(entry.path)) { // look into package.json
      if (entry.type === 'Directory') return;
      ...
      bufferStream.on('data', function(chunk) {
        ...
        //according to https://docs.npmjs.com/cli/v6/configuring-npm/folders#prefix-configuration
        // this is current designated npm behaviour by putting things under node_modules/<package-name> if we use --prefix flag
        modulePath = path.join(installBasePath, '/node_modules', name);
      }).on('error', function(err) {
        return callback(new CdifError('READ_MODULE_FAIL', err.message));
      });
