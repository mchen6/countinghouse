  ...
  tar.on('entry', function(entry) {
    if (/package\.json$/.test(entry.path)) { // look into package.json
      ...
      bufferStream.on('data', function(chunk) {
        ...
        var installBasePath = path.join(input.path, name);
        ...
        var modulePath = path.join(installBasePath, '/node_modules', name);

        var doFreshNPMInstall = true;
        ...
        if (options.vscode === true && fs.existsSync(modulePath)) {
          try {
            ...
            if (savedDesignID === input.apiDesignID) doFreshNPMInstall = false;
          } catch(e) {
        ...
        if (doFreshNPMInstall === false) {
          ...
          return _this.loadModuleUnsafe(installBasePath, modulePath, name, version, packageInfo, input.apiDesignID, callback);
        }
        ...
        try {
          exec(command, {timeout: 120000}, function(err, stdout, stderr) {
            ...
            _this.loadModuleUnsafe(installBasePath, modulePath, name, version, packageInfo, input.apiDesignID, callback);
          });
        } catch (e) {
...
ModuleManager.prototype.loadModuleUnsafe = function(installBasePath, modulePath, name, version, packageInfo, apiDesignID, callback) {
  var _this = this;
  var moduleInstance = null;
  ...
  unsafeDomain.run(function() {
    _this.loadModuleFromPath(modulePath, name, version, function(e, mi) {
      ...
      chmodr(installBasePath, 0666, function(err) {
        ...
        fs.writeFileSync(path.join(modulePath, 'package.json'), JSON.stringify(packageInfo, null, 2), 'utf8');
        ...
        setTimeout(function() {
          //get the list of device objects which belongs to moduleInstance
          //this event is handled by device manager
          _this.emit('querydevicelist', moduleInstance, packageInfo, function(err, info) {
