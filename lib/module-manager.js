...
var McpForgeUtil       = require('../lib/mcpforge-util');
var options        = require('../lib/cli-options');
var exec           = require('child_process').exec;
var McpForgeError      = require('../lib/mcpforge-error').McpForgeError;
...
ModuleManager.prototype.onDeviceOnline = function(packageInfo, modulePath, device, module) {
  ...
  if (found === false) {
    LOG.E(new McpForgeError('CORRESPONDING_MODULE_NOT_FOUND', 'device online'));
  }
};
...
ModuleManager.prototype.onDeviceOffline = function(device, module) {
  ...
  if (found === false) {
    LOG.E(new McpForgeError('CORRESPONDING_MODULE_NOT_FOUND', 'device offline'));
  }
};
...
ModuleManager.prototype.getModuleInfoFromPath = function(pathList) {
  ...
  if (typeof(pathList) === 'string') {
    ...
    LOG.E(new McpForgeError('LOAD_MODULE_FAIL', 'invalid loadModule argument:', pathList));
    return null;
  }
  ...
  list.forEach(function(item) {
    if (fs.existsSync(item) && fs.existsSync(item + '/package.json')) {
      try {
        ...
        LOG.E(new McpForgeError('LOAD_MODULE_FAIL', 'invalid package.json file:', item));
      }
    } else {
      LOG.E(new McpForgeError('LOAD_MODULE_FAIL', 'invalid path or package.json file not found:', item));
    }
  });
  return info;
};
...
ModuleManager.prototype.loadAllModules = function() {
  ...
  if (options.workerThread !== true || isMainThread === true) {
    ...
    deviceDB.getAllModuleInfo(function(err, data) {
      if (err) {
        return LOG.E(new McpForgeError('GET_MODULE_INFO_FAIL', err.message));
      }
...
ModuleManager.prototype.loadModuleFromPath = function(modulePath, name, version, callback) {
  if (options.workerThread === true && isMainThread === true) {
    return this.loadModuleByWorker(modulePath, name, version, function(err, mi) {
      // if (err) LOG.E(new McpForgeError('LOAD_MODULE_FAIL', err.message));
  ...
  if (typeof(modulePath) !== 'string' || !fs.existsSync(modulePath)) return callback(new McpForgeError('INVALID_MODULE_PATH', modulePath));
  ...
  try {
    ...
    LOG.E(new McpForgeError('LOAD_MODULE_FAIL', name, e.message, e.stack));
    return callback(e);
  }
  ...
  getDeviceConfig(options, name, function(err) {
    if (err) LOG.E(new McpForgeError('DEVICE_CONFIG_LOAD_FAIL', err.message));
...
ModuleManager.prototype.reloadModule = function(modulePath, callback) {
  ...
  try {
    ...
    if (typeof(name) !== 'string') {
      return callback(new McpForgeError('MODULE_PACKAGE_NAME_TYPE_ERROR', name));
    }
    if (typeof(version) !== 'string' || semver.valid(version) == null) {
      ...
      this.unloadModule(name, 'RELOAD MODULE FAIL DUE TO INVALID VERSION STRING', function() {
        return callback(new McpForgeError('MODULE_VERSION_INFO_INVALID', version));
      });
    }
    ...
    this.unloadModule(name, 'RELOAD MODULE FAIL DUE TO EXCEPTION: ' + e.message, function() {
      return callback(new McpForgeError('LOAD_MODULE_FAIL', e.message, e.stack));
    });
  }
};
...
ModuleManager.prototype.restartModule = function(modulePath, name, version, callback) {
  var m = this.modules[name];
  if (m == null) {
    return callback(new McpForgeError('RESTART_MODULE_FAIL', 'unknown module', name));
  }
...
ModuleManager.prototype.verifyModule = function(input, callback) {
  ...
  if (typeof(input.name) !== 'string') {
    return callback(new McpForgeError('MODULE_NAME_TYPE_ERROR', input.name));
  }
  if (input.path == null || typeof(input.path) !== 'string') {
    return callback(new McpForgeError('MODULE_INSTALL_PATH_PREFIX_INVALID'));
  }
  ...
  file.on('error', function(e) {
    return callback(new McpForgeError('READ_MODULE_FAIL', e.message));
  });

  var unzip = file.pipe(zlib.Unzip());
  unzip.on('error', function(e) {
    return callback(new McpForgeError('UNZIP_MODULE_FAIL', e.message));
  });

  var tar = unzip.pipe(tar.Parse());
  tar.on('error', function(e) {
    return callback(new McpForgeError('UNTAR_MODULE_FAIL', e.message));
  });
  ...
  tar.on('end', function() {
    try {
      ...
      return callback(new McpForgeError('MODULE_INSTALL_FAIL', e.message));
    }

    if (typeof(packageInfo) !== 'object') {
      return callback(new McpForgeError('MODULE_PACKAGE_INFO_TYPE_ERROR'));
    }
    name    = packageInfo.name;
    version = packageInfo.version;
...
ModuleManager.prototype.loadModuleUnsafe = function(modulePath, name, version, packageInfo, apiDesignID, callback) {
  ...
  unsafeDomain.on('error', function(err) {
    return callback(new McpForgeError('MODULE_INSTALL_FAIL', err.stack), {packageInfo: packageInfo, deviceList: [], moduleInstallPath: modulePath});
  });

  if (options.cloud9 === true) {
    C9Launcher.startCloud9(modulePath, McpForgeUtil.getHostIp(), options.dbUrl, apiDesignID, function(errr) {
      if (errr) return callback(new McpForgeError('MODULE_INSTALL_FAIL', errr.message));
    ...
    VSCodeLauncher.startVSCode(modulePath, apiDesignID, function(errr) {
      if (errr) return callback(new McpForgeError('MODULE_INSTALL_FAIL', errr.message), {packageInfo: packageInfo, deviceList: [], moduleInstallPath: modulePath});
      unsafeDomain.run(function() { _this.loadModuleAndQueryDevice(modulePath, name, version, packageInfo, callback); });
    });
  } else {
    unsafeDomain.run(function() { _this.loadModuleAndQueryDevice(modulePath, name, version, packageInfo, callback); });
...
ModuleManager.prototype.loadModuleAndQueryDevice = function(modulePath, name, version, packageInfo, callback) {
  this.loadModuleFromPath(modulePath, name, version, function(e, mi) {
    if (e != null) return callback(new McpForgeError('MODULE_INSTALL_FAIL', name, e.message, e.stack), {packageInfo: packageInfo, deviceList: [], moduleInstallPath: modulePath});
    moduleInstance = mi;
...
ModuleManager.prototype.onQueryDeviceListResult = function(error, deviceList, packageInfo, callback) {
  if (packageInfo != null && packageInfo.name != null) {
    ...
    return callback(new McpForgeError('MODULE_PACKAGE_INFO_INVALID'));
  }
};
