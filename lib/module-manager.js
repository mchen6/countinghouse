...
ModuleManager.prototype.publishModule = function(options, callback) {
  if (options.user == null || options.pass == null || typeof(options.user) !== 'string' || typeof(options.pass) !== 'string') {
    return callback(new CdifError('must provide correct user credential'));
  }

  if (typeof(options.path) !== 'string') {
    return callback(new CdifError('package path incorrect: ' + options.path));
  }
  ...
  if ((registry != null) && (typeof(registry) !== 'string' ||
    ((/^http:\/\/.{1,256}$/.test(registry) ||
     /^https:\/\/.{1,256}$/.test(registry)) === false))) {
    return callback(new CdifError('module publish: invalid registry name: ' + options.registry));
  }

  try {
    fs.accessSync(options.path, fs.W_OK);
  } catch (e) {
    return callback(new CdifError('cannot access package folder: ' + options.path + ' reason: ' + e.message));
  }
