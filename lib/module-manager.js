...
ModuleManager.prototype.installModuleFromRegistry = function(registry, name, version, callback) {
  ...
  var command = null;
  // using fixed prefix for now
  var prefix  = '/tmp/' + name + '@' + version;

  try {
    mkdirp.sync(prefix);
    fs.accessSync(prefix, fs.W_OK);
  } catch (e) {
    return callback(new CdifError('cannot create module folder: ' + prefix + ' reason: ' + e.message));
  }

  if (registry == null) {
    command = 'npm install ' + '--prefix' + prefix + ' ' + name + '@' + version;
  } else {
    command = 'npm install ' + '--registry=' + registry + ' ' + '--prefix ' + prefix + ' ' + name + '@' + version;
  }

  try {
    exec(command, {timeout: 60000}, function(err, stdout, stderr) {
      if (err) {
        rimraf(prefix, function (e) {
          if (err) {
            LOG.E(new CdifError(e.message));
          }
        ...
        var modulePath = prefix + '/node_modules/' + name;
        this.addModuleInformation(name, version, modulePath, function(e) {
          if (e) {
            return callback(new CdifError('add module record failed: ' + name + ', reason: ' + e.message), null);
          }
          if (this.loadModuleFromPath(modulePath, name, version) == null) {
            return callback(new CdifError('load module failed: ' + name));
          }
...
ModuleManager.prototype.uninstallModule = function(name, callback) {
  ...
  this.getModuleInformation(name, function(e, data) {
    if (e) {
      return callback(new CdifError('get module record failed: ' + name + ', reason: ' + e.message), null);
    }

    if (data == null) {
      return callback(new CdifError('unknown module: ' + name), null);
    }

    // using fixed prefix for now
    var prefix  = '/tmp/' + data.name + '@' + data.version;
    var command = 'npm uninstall ' + '--prefix ' + prefix + ' ' + name;
...
ModuleManager.prototype.addModuleInformation = function(name, version, path, callback) {
  if (version === '') version = 'latest';

  deviceDB.setModuleInfo(name, version, path, function(err) {
    if (err) {
...
ModuleManager.prototype.getModuleInformation = function(name, callback) {
  deviceDB.getModuleInfo(name, function(err, data) {
    if (err) {
      return callback(new Error('cannot get module info for: ' + name + ' reason: ' + err.message));
    }
