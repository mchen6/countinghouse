...
var McpForgeError   = require('./mcpforge-error').McpForgeError;
var DeviceError = require('./mcpforge-error').DeviceError;
...
util.inherits(McpForgeDevice, events.EventEmitter);


McpForgeDevice.prototype.setAction = function(serviceID, actionName, action) {
...
McpForgeDevice.prototype.initServices = function() {
...
McpForgeDevice.prototype.getDeviceSpec = function(session) {
  var cb = null;
  ...
  if (this.spec === null) {
    return cb(new McpForgeError('CANNOT_GET_DEVICE_SPEC'), null);
  }
...
McpForgeDevice.prototype.getServiceStates = function(serviceID, session) {
  if (session == null) {
...
McpForgeDevice.prototype.setServiceStates = function(serviceID, values, callback) {
  if (callback == null || typeof(callback) !== 'function') {
    return LOG.DE(this, new McpForgeError('GET_SERVICE_STATE_FAIL_INVALID_CALLBACK'));
  }
  var service = this.services[serviceID];
  if (service == null) {
...
McpForgeDevice.prototype.connect = function(user, pass, callback) {
  if (this.connectionState === 'redirecting') {
    return callback(new McpForgeError('DEVICE_IN_ACTION'), null, null);
  }
...
McpForgeDevice.prototype.disconnect = function(session) {
  return this.connMan.processDisconnect(session.callback);
};

McpForgeDevice.prototype.getHWAddress = function(callback) {
...
McpForgeDevice.prototype.deviceControl = function(serviceID, actionName, args, session) {
  var service = this.services[serviceID];
  if (service == null) {
    if (typeof(session) === 'function') {
...
McpForgeDevice.prototype.invokeDeviceCallback = function(path, data, session) {
  var cb = null;
  ...
  this._deviceCallbackHandler(path, data, function(err, output) {
    if (err != null) {
      var error = null;
      if (err instanceof DeviceError || err instanceof McpForgeError) {
        error = err;
      } else {
...
McpForgeDevice.prototype.updateDeviceSpec = function(newSpec) {
...
McpForgeDevice.prototype.setEventSubscription = function(serviceID, subscribe, unsubscribe) {
...
McpForgeDevice.prototype.subscribeDeviceEvent = function(serviceID, actionName, input, inputKey, callback) {
  var service = this.services[serviceID];
  if (service == null) {
...
McpForgeDevice.prototype.unsubscribeDeviceEvent = function(serviceID, actionName, input, inputKey, callback) {
  var service = this.services[serviceID];
  if (service == null) {
    return callback(new DeviceError('SERVICE_NOT_FOUND', serviceID));
  }
...
McpForgeDevice.prototype.getDeviceRootUrl = function(session) {
...
McpForgeDevice.prototype.getDeviceRootSchema = function() {
...
McpForgeDevice.prototype.destroyCdifDevice = function() {
...
McpForgeDevice.prototype.resolveSchemaFromPath = function(path, self, callback) {
  ...
  if (/^\/./.test(path) === false) {
    return callback(new McpForgeError('INVALID_JSON_POINTER'), self, null);
  }
  ...
  parser.dereference(doc, {$refs: {external: false}}, function(err, out) {
    if (err) {
      return callback(new McpForgeError('POINTER_DEREF_ERROR', err.message), self, null);
    }
    callback(null, self, out.__);
  });
};

McpForgeDevice.prototype.setOAuthAccessToken = function(params, session) {
  if (typeof(this._setOAuthAccessToken) === 'function') {
    this._setOAuthAccessToken(params, function(err) {
      if (err) {
        return session.callback(new McpForgeError(err.message));
      }
      this.connectionState = 'connected';
      session.callback(null);
    }.bind(this));
  } else {
    session.callback(new McpForgeError('CANNOT_SET_OAUTH_ACCESS_TOKEN_INVALID_INTERFACE'));
  }
};
...
McpForgeDevice.prototype.oauthTokenValidate = function(session, callback) {
