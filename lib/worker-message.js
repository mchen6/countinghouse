...
WorkerMessage.prototype.sendMessageToWorker = function(message, callback) {
  if (this.worker != null) {
    ...
    console.log('msgCommand: ' + message.command);
...
WorkerMessage.prototype.sendMessageToParent = function(id, err, data) {
  ...
  if (err) return parentPort.postMessage({id: id, errMsg: err.message, data: sData});
  return parentPort.postMessage({id: id, errMsg: null, data: sData});
};
