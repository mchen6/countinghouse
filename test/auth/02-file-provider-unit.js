...
describe('auth 02: FileAuthProvider ergonomics (demo key generation, COUNTINGHOUSE_API_KEY)', () => {
  const configPath = `/tmp/countinghouse-test-auth-02-${process.pid}.json`;
  ...
  it('generates and persists a demo key with wildcard access when the config file does not exist', (done) => {
    if (fs.existsSync(configPath)) fs.unlinkSync(configPath); // paranoia: guarantee a true first-run

    const provider = new FileAuthProvider({configPath: configPath});
    ...
    provider.authenticate(demoKey, 'any-device-id', null, null, (err, result) => {
      ...
      // already using.
      const provider2 = new FileAuthProvider({configPath: configPath});
      provider2.authenticate(demoKey, 'any-device-id', null, null, (err, result) => {
  ...
  it('does not overwrite an existing (even empty) auth.json with a demo key', (done) => {
    ...
    provider.authenticate('anything', 'any-device-id', null, null, (err, result) => {
      assertDenied(err, result, 'an empty auth.json should authorize nobody');
      done();
    });
  });

  it('COUNTINGHOUSE_API_KEY grants wildcard access without requiring auth.json to exist', (done) => {
    ...
    provider.authenticate('env-mode-key', 'any-device-id', null, null, (err, result) => {
      ...
      provider.authenticate('some-other-key', 'any-device-id', null, null, (err, result) => {
  ...
  it('auth.json\'s optional "admin" field controls isAdmin, independent of device access', (done) => {
    const fileKeyConfig = {};
    ...
    provider.authenticate('plain-key', null, null, null, (err, result) => {
      ...
      provider.authenticate('admin-key', 'some-other-device-not-granted', null, null, (err, result) => {
  ...
  it('COUNTINGHOUSE_API_KEY and auth.json entries both work at the same time', (done) => {
    const fileKeyConfig = {};
    ...
    provider.authenticate('env-mode-key', 'any-device-id', null, null, (err, result) => {
      ...
      provider.authenticate('file-key', 'some-device-id', null, null, (err, result) => {
