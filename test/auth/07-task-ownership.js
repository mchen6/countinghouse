...
var AUTH_CONFIG_PATH = '/tmp/countinghouse-test-auth-07-' + process.pid + '.json';

var ALICE   = 'alice-key';    // owns the tasks created below
var MALLORY = 'mallory-key';  // valid key, wildcard device access, owns nothing
var ADMIN   = 'admin-key';    // may read across tenants
var ECHO_TOOL = 'echo_device_echoservice_echo';
...
describe('auth 07: MCP tasks/* and the HTTP job routes enforce per-tenant job ownership', function() {
  ...
  it('alice creates a task-augmented tools/call', function(done) {
    mcp(ALICE, {
  ...
  it('alice can read the result of her own task', function(done) {
    mcp(ALICE, {jsonrpc: '2.0', id: 2, method: 'tasks/result', params: {taskId: aliceTaskId}}, function(err, res) {
      if (err) return done(err);
  ...
  it('mallory CANNOT read alice\'s task result', function(done) {
    mcp(MALLORY, {jsonrpc: '2.0', id: 3, method: 'tasks/result', params: {taskId: aliceTaskId}}, function(err, res) {
      if (err) return done(err);
      var text = JSON.stringify(res.body);
      if (text.indexOf('ALICE-CONFIDENTIAL') !== -1) {
  ...
  it('mallory CANNOT read alice\'s task metadata via tasks/get', function(done) {
    mcp(MALLORY, {jsonrpc: '2.0', id: 4, method: 'tasks/get', params: {taskId: aliceTaskId}}, function(err, res) {
      if (err) return done(err);
      if (res.body.error == null) {
  ...
  it('mallory\'s tasks/list does NOT include alice\'s task', function(done) {
    mcp(MALLORY, {jsonrpc: '2.0', id: 5, method: 'tasks/list'}, function(err, res) {
      if (err) return done(err);
      ...
      var leaked = res.body.result.tasks.filter(function(t) { return t.taskId === aliceTaskId; });
      if (leaked.length > 0) {
  ...
  it('alice\'s own tasks/list DOES include her task', function(done) {
    mcp(ALICE, {jsonrpc: '2.0', id: 6, method: 'tasks/list'}, function(err, res) {
      if (err) return done(err);
      var found = res.body.result.tasks.filter(function(t) { return t.taskId === aliceTaskId; });
      if (found.length !== 1) {
  ...
  it('an admin key CAN read across tenants', function(done) {
    mcp(ADMIN, {jsonrpc: '2.0', id: 7, method: 'tasks/get', params: {taskId: aliceTaskId}}, function(err, res) {
      if (err) return done(err);
  ...
  ['tasks/get', 'tasks/result', 'tasks/cancel'].forEach(function(method) {
    it('no apiKey at all is refused on ' + method, function(done) {
      mcp(null, {jsonrpc: '2.0', id: 8, method: method, params: {taskId: aliceTaskId}}, function(err, res) {
        if (err) return done(err);
        var text = JSON.stringify(res.body);
        if (res.body.error == null) {
  ...
  it('no apiKey at all is refused on tasks/list (no enumeration)', function(done) {
    mcp(null, {jsonrpc: '2.0', id: 9, method: 'tasks/list'}, function(err, res) {
      if (err) return done(err);
      if (res.body.error == null) {
  ...
  it('HTTP /get-job: mallory cannot read alice\'s job', function(done) {
  ...
  it('HTTP /remove-job: mallory cannot delete alice\'s job', function(done) {
  ...
  it('HTTP /get-job-history: mallory gets no entries for alice\'s job name', function(done) {
  ...
  it('alice\'s job survived every one of mallory\'s attempts', function(done) {
    mcp(ALICE, {jsonrpc: '2.0', id: 10, method: 'tasks/get', params: {taskId: aliceTaskId}}, function(err, res) {
      if (err) return done(err);
      if (res.body.result == null || res.body.result.taskId !== aliceTaskId) {
  ...
  it('the owner can cancel her own task', function(done) {
    mcp(ALICE, {jsonrpc: '2.0', id: 11, method: 'tasks/cancel', params: {taskId: aliceTaskId}}, function(err, res) {
      if (err) return done(err);
...
var DEVICE_ID = 'c5284c70-ae5f-591c-b2f1-cf0b4ebd0767';
