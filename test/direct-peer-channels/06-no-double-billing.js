...
const COMPOSITE_DEVICE_ID = '6042a93c-06b1-54b5-a62d-a68c15f1ce1e'; // composite-demo
const ECHO_DEVICE_ID      = 'c5284c70-ae5f-591c-b2f1-cf0b4ebd0767'; // echo-device-module, called directly for the outer-charge baseline
const INTERNAL_API_KEY    = 'composite-demo-internal'; // composite-demo/device.js's fixed internal identity
...
const SETTLE_POLL_MS    = 200;   // gap between balance reads
const SETTLE_STABLE     = 3;     // identical consecutive reads that count as quiesced
const SETTLE_TIMEOUT_MS = 15000; // overall cap before giving up
...
function runBillingAssertion(done) {
  measureOuterCallCost((err, outerCallCost) => {
  ...
  settledBalance(null, (err, before) => {
    ...
    invokeComposite((err, body) => {
      ...
      settledBalance(before, (err, after) => {
        ...
        const hopCost = delta - outerCallCost;
        if (hopCost !== 2) {
...
describe('direct-peer-channels 06: composite-demo does not double-bill (--directPeerChannels on)', function() {
  ...
  after((done) => {
    request(url).post('/shutdown').set('X-CH-Key', INTERNAL_API_KEY).end(() => { done(); }); // /shutdown is now admin-gated
  });

  it('a 2-hop composite call bills exactly 2, and bill still shows 2 independent records', (done) => {
