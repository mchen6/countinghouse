...
describe('direct-peer-channels 04: automatic metering on the direct path (D5)', function() {
  ...
  after((done) => {
    request(url).post('/shutdown').set('X-CH-Key', 'aabbcc').end(() => { done(); }); // /shutdown is now admin-gated
  });

  const ECHO_DEVICE_CLIENT_ID = 'efefb416-bdc0-54eb-96a9-38f96f52779d'; // echo-device-client-module
  const ECHO_DEVICE_ID        = 'c5284c70-ae5f-591c-b2f1-cf0b4ebd0767'; // echo-device-module, called directly for the baseline below
  ...
  // total.
  let outerCallCost = null;

  it('a plain (non-composing) call establishes the outer invoke-action charge', (done) => {
  ...
  it('a call over a direct peer channel bills the hop exactly once, not zero or twice', (done) => {
    settledBalance(null, (err, before) => {
      if (err) return done(err);
