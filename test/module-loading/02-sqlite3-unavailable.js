...
var AUTH_CONFIG_PATH = '/tmp/countinghouse-test-nosqlite-' + process.pid + '.json';
var PRELOAD          = './test/fixtures/no-sqlite3-preload.js';

var KEY   = 'nosqlite-key-' + process.pid;
var OTHER = 'nosqlite-other-' + process.pid;
...
describe('module-loading 02: with sqlite3 unloadable, the file backend still works end to end', function() {
  ...
  after(function(done) {
    ...
    exec('pkill -f "' + AUTH_CONFIG_PATH + '"', function() { done(); });
  });

  it('the server starts at all (the documented --loadModule path never touches the registry DB)', function(done) {
  ...
  it('tools/list works and is still filtered per apiKey', function(done) {
    request(url).post('/mcp').set('Content-Type', 'application/json').set('X-CH-Key', KEY)
      ...
      .end(function(err, res) {
        ...
        request(url).post('/mcp').set('Content-Type', 'application/json').set('X-CH-Key', OTHER)
          ...
          .end(function(err, res2) {
            if (err) return done(err);
            var n2 = res2.body.result.tools.map(function(t) { return t.name; });
  ...
  it('tools/call round-trips', function(done) {
    request(url).post('/mcp').set('Content-Type', 'application/json').set('X-CH-Key', KEY)
  ...
  it('HTTP invoke-action works and is metered', function(done) {
    request(url).get('/balance').set('X-CH-Key', KEY).end(function(err, res) {
      if (err) return done(err);
      var before = res.body.balance;
      request(url).post('/devices/' + DEVICE_ID + '/invoke-action').set('X-CH-Key', KEY)
        ...
        .expect(200, function(err) {
          if (err) return done(err);
          setTimeout(function() {
            request(url).get('/balance').set('X-CH-Key', KEY).end(function(err, res2) {
              if (err) return done(err);
  ...
  it('authorization still denies an unauthorized key', function(done) {
    request(url).post('/mcp').set('Content-Type', 'application/json').set('X-CH-Key', OTHER)
  ...
  it('selecting the sqlite backend fails with an actionable message, not a loader stack', function(done) {
