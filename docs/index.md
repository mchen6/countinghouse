...
Measured against a stdio subprocess and localhost HTTP, at 1KB the gap nearly closes: 0.33ms versus 0.33ms versus 0.77ms per hop. At 1MB it does not. One hop costs 8.8ms in-process, 82.5ms over stdio, 37.8ms over HTTP; a hundred hops cost 0.9 seconds, 8.3 seconds, and 3.4 seconds respectively, and 19s, 166s and 68s of CPU. The stdio condition is a bare JSON-RPC echo with no handshake and no `tools/list` — the best case for stdio, not a typical one. [Full tables, methodology and caveats.](https://github.com/mchen6/countinghouse/blob/master/docs/direct-peer-channels.md#benchmark-transport-overhead-vs-other-tool-to-tool-mechanisms)

Now the honest part, which the benchmark script computes rather than asserts: the largest per-hop saving it measured is 29ms, so a tool whose own execution takes more than about 290ms would see even that as under 10% of the call. Anything hitting a database, an API or a model is far past that line. Per call, the transport rounds to nothing.

Per session it doesn't. Agent runs are not one call; they are hundreds, over hours, and the arithmetic is simply multiplication. That 8.3-versus-0.9-second row is one chain. The workload where this shows up at all is composition-heavy, large-payload and fast-tooled — parse, chunk, embed, retrieve, rerank — and the CPU column matters as much as the clock: 166 seconds versus 19 is not latency you wait out, it is capacity you buy.
