...
**Relationship to `test/fixtures/route-inventory.json`.** That file is the
complete, mechanically-verified list of every HTTP path currently mounted —
`test/module-loading/11-route-inventory.js` diffs the live route table
against it and fails the suite on any drift, in either direction. This
matrix is a different thing: the per-path guarantee record, filled in by
hand, one row per path, for the concerns above. **The two are not yet in
sync** — this matrix does not currently have a row for every path the JSON
inventory lists. As of 7.0.0, `/devices/:deviceID/get-spec`,
`/devices/:deviceID/schema` and `/device-list`
are all live, mounted, and present in the inventory, but have no row below.
This is a known gap, recorded here rather than implied away, not a claim
that those three are unguarded — closing it is separate work from this
change.
...
|---|---|---|---|---|---|---|
| **MCP `tasks/get`, `tasks/result`, `tasks/list`, `tasks/cancel`** | ✅ **fixed 2026-08-11 — previously none at all.** These four took no `appKey` parameter, so there was nothing in scope to check: an anonymous caller could read any tenant's task output, enumerate every task, and delete tasks it didn't create (verified live, not inferred). Now `resolveCallerIdentity` (`lib/mcp/gateway.js`) resolves the key through the same `doUserAuth`/AuthProvider path every other entry path uses, and `lib/job-control.js` enforces per-job ownership: the creator's authenticated apiKey, `isAdmin` may cross tenants, `tasks/list` filters to the caller's own. Unknown/absent key ⇒ refused. A job with no recorded owner is admin-only. Test: `test/auth/07-task-ownership.js` | ➖ exempt — these methods read and cancel existing tasks; they carry no action payload to validate. The underlying action's input was validated at `tools/call` time | ➖ exempt — reading or cancelling a task is not itself a billable action. The task's own execution is metered on the task-augmented `tools/call` row above | ✅ **fixed in 7.0.0 — previously none.** All four now go through `resolveGatedIdentity` (`lib/mcp/gateway.js`), which pairs `resolveCallerIdentity` with the shared per-apiKey check in `lib/rate-limit-gate.js`. The two are deliberately one helper rather than a repeated pair, so a fifth `tasks/*` method cannot be added later that resolves identity and forgets the limit. Keyed on the *resolved* `authCtx.appKey`, never the header. Shares the `--apiKeyRateLimit` budget with tool calls, so a client polling `tasks/get` draws down its own call allowance — a deliberate choice (one caller, one request budget) over a second flag. Denied as `JSONRPC_INTERNAL_ERROR` with message `rate limit exceeded`, the same code these handlers already answer an auth failure with, since JSON-RPC 2.0 defines none for this condition. Test: `test/auth/15-read-path-rate-limits.js` | ➖ exempt — no device call is made | JSON-RPC error object (HTTP 200 per JSON-RPC 2.0 framing), same shape `tools/call`'s own `userAuth` failure already used. `tasks/get`/`result` deliberately return the *same* `unknown task` message for "doesn't exist" and "isn't yours", so taskIds can't be enumerated by probing |
| **HTTP `POST /devices/:deviceID/{get,remove}-job`, `get-job-history`** | ✅ **fixed 2026-08-11 — two layers.** `lib/routes/user.js`'s per-request `userAuth` (device access) as before, plus the same per-job ownership check `tasks/*` uses, applied in `lib/job-control.js` rather than at either front door — gating only the MCP gateway would have left these three as the bypass. Closes this route's own long-standing in-code TODO ("arbitrary user can remove someone else's jobs"). **Known narrowing**: removing a repeatable job *scheduler* (`isRepeat: true`) now requires admin — bullmq's scheduler record carries no owner field to match against, so an unattributable delete is restricted to the operator rather than left open. Per-scheduler ownership would need its own persisted record; deliberately not decided here. Test: `test/auth/07-task-ownership.js` | ➖ exempt — same reason as the MCP row above | ➖ exempt for the read routes. `add-job` is metered on job completion (`job-control.js`), billed to the authenticated creator | ✅ **fixed in 7.0.0. This cell used to read "inherited from `lib/routes/user.js` → the standard route stack", which was false**: `lib/routes/user.js` runs `userAuth` and nothing else, and no rate-limit middleware is or ever was mounted in `lib/route-manager.js`. The only pre-existing per-apiKey limiter lives inside `CdifInterface.prototype.invokeDeviceAction`, which none of these three routes calls — so the guarantee this table asserted for them did not exist. That is this document's founding bug shape again, this time in the table rather than in the code. Now really enforced, via `rateLimitGate.guard` in each route (`lib/routes/{get-job,get-job-history,remove-job}.js`) on the authenticated `session.appKey`. The job's *execution* does additionally pass through `invokeDeviceAction`'s limiters, which was the true half of the old claim. Test: `test/auth/15-read-path-rate-limits.js` | ➖ exempt — no synchronous device call | `{topic, code, message}` JSON body via `Session.prototype.callbackWithoutTimer`, `code` = `JOB_ACCESS_DENIED` for a non-owner |
| **HTTP `POST /devices/:deviceID/add-job`** | ✅ `lib/routes/user.js` → `user-auth.js`, same as every other device-scoped route. **Fixed 2026-08-11**: the job's owner/billing subject is now the authenticated caller, passed out-of-band as `authCtx`; previously it was read from the request body's `opts.apiKey`, letting any valid key bill and impersonate any other (verified: alice's request moved mallory's balance). The route additionally whitelists the scheduling fields it accepts. Test: `test/auth/08-job-billing-identity.js` | ✅ at execution time, via the same shared `validateActionCall` | ✅ on job completion (`job-control.js`'s `initJobProcess`), against `jobData.apiKey` — now necessarily the authenticated creator | ✅ **fixed in 7.0.0, and this one was a live bypass, not only a wrong cell.** It used to read "route stack" — false for the same reason as the row above. It mattered more here: MCP task creation *is* rate-limited, deliberately at creation time because unbounded queue growth is the resource being protected (`createTaskForToolCall`, `lib/mcp/gateway.js`), and this route creates the very same jobs. A caller could therefore queue jobs without limit simply by using HTTP instead of MCP, making the MCP-side limit optional. Now gated by `rateLimitGate.guard` on the authenticated `session.appKey`, sharing one per-apiKey budget with every other gated path. Test: `test/auth/15-read-path-rate-limits.js` | ✅ bullmq `job.opts.timeout`, reimplemented via `setTimeout` | `{topic, code, message}` JSON body |
| **HTTP `GET /balance`** and the **`countinghouse_check_balance` MCP tool** | ✅ **fixed 2026-08-11 — previously none.** Both handed the raw header value to `MeteringProvider.checkBalance` without consulting AuthProvider, so any invented string got `200 {"balance": 0}` — an unauthenticated balance oracle. Both now go through `doUserAuth` with `deviceID: null` (a balance belongs to an apiKey, not a device) and read back the *authenticated session's* appKey, so a caller can only ever ask about itself. Test: `test/auth/10-balance-auth.js` | ➖ exempt — no action payload | ➖ exempt — reading a balance is not a billable call | ✅ **fixed in 7.0.0 — previously none.** Both paths were authenticated but unlimited, one Redis round trip per request and pollable as fast as an authenticated caller liked. Both now call the shared gate: `lib/routes/balance.js` via `rateLimitGate.guard` (HTTP `429` with `code: RATE_LIMIT_EXCEEDED`), the MCP tool via `rateLimitGate.check` inside `handleToolsCall`, denied as a tool *error* (`isError`, `structuredContent.code`) rather than a JSON-RPC one, matching how the task-creation limiter already denies. Both key on the resolved session's `appKey`, not the header — the same reason the balance read itself does, so one key cannot spend another's budget. Test: `test/auth/15-read-path-rate-limits.js` | ➖ exempt | HTTP: `403 {topic, code, message}` with `code: SYSTEM_ERROR_UNKNOWN_USER`. MCP: `toolCallResult()`'s error branch, no balance value present anywhere in the body |
| **HTTP module-lifecycle routes** (`/load-module`, `/unload-module`, `/restart-module`, `/reload-module`, `/shutdown`) | ✅ `lib/routes/admin-only.js`, mounted ahead of each route in `lib/route-manager.js`. Requires `isAdmin` on the resolved session — a *separate* capability from device access, so a `devices: ['*']` key is still refused. Previously mounted only under `--debug`/`--verifyModule`; now always mounted and gated per request. Tests: `test/auth/06-admin-gating.js`, `test/auth/11-admin-module-lifecycle.js` | ➖ exempt — no device action; these take a module path/name | ➖ exempt — operator actions, not tenant calls | ❌ none. Deliberate: these are operator endpoints, and rate-limiting an operator out of `/shutdown` during an incident is the wrong failure mode | ➖ exempt | `403 {topic, code, message}` with `code: ADMIN_REQUIRED` (or `SYSTEM_ERROR_UNKNOWN_USER` for a key AuthProvider doesn't know) |
| **HTTP `/callbacks/:deviceID/*`** | ➖ **removed in 7.0.0.** It was genuinely unauthenticated and reachable whenever the server was — mounted ahead of `lib/routes/user.js` with the code saying so ("callback don't do user auth") — which is what put it on the pre-release audit's list as needing its own review before any public deployment. That review is this row. **It was removed as dead code, not as a working feature**, the same way the event subsystem went in 5.0.0: the chain ran `routes/callbacks.js` → `CdifInterface.invokeDeviceCallbacks` → `DeviceManager.onInvokeDeviceCallback` → `CHDevice.invokeDeviceCallback` → `this._deviceCallbackHandler`, and **nothing in the repo ever assigned `_deviceCallbackHandler`** — the two references in `lib/countinghouse-device.js` both only read it. So every request that ever reached this route hit the null check and came back `DEVICE_CALLBACK_NOT_AVAILABLE`. No module, example, test or doc referenced it. The original purpose was a CDIF-era inbound webhook; MCP has no equivalent concept, and since 5.0.0 removed event delivery there is no path from an inbound HTTP request to a connected client anyway. Whoever reinstates inbound webhooks needs to supply both halves — a way for a module to register a handler, and somewhere for the result to go. Test: `test/auth/14-removed-callback-routes.js` asserts the route is now 404 | ➖ | ➖ | ➖ | ➖ | ➖ |
| **HTTP `GET /callback_url`** (the OAuth redirect target) | ➖ **removed in 7.0.0**, together with the rest of the framework's OAuth device path (`lib/oauth/`, the `oauth` npm dependency, `CHDevice.setOAuthAccessToken`, `CHDevice.oauthTokenValidate`, and the `oauth_version` branch in `DeviceManager.onDeviceOnline`). It never had a row here, and it never worked: `lib/routes/oauth-callback.js` called `cdifInterface.setDeviceOAuthAccessToken(...)`, **a method that was never defined on `CdifInterface`**, so every GET threw `TypeError` and answered 500. It also read `req.session`, which only `lib/routes/user.js` and `lib/routes/admin-only.js` ever set, and neither was mounted on this path. The subsystem behind it had no reachable entry point either — nothing outside `lib/oauth/oauth.js` itself ever set the `oauth_version` that `onDeviceOnline` branched on, and `oauthTokenValidate` had no callers at all. **The one real capability this removes is a module holding a third-party API's OAuth token** — MCP's authorization framework covers client→server (which is what AuthProvider does here) and deliberately leaves server→third-party credentials to the implementation. That gap is now unfilled rather than filled-but-broken, and needs its own design when a module needs it. Test: `test/auth/14-removed-callback-routes.js` | ➖ | ➖ | ➖ | ➖ | ➖ |
...
| **`/devices/:deviceID/connect`, `/devices/:deviceID/disconnect`** | ➖ **removed in 7.0.0.** `lib/routes/connect.js` called `cdifInterface.connectDevice`, and `disconnect.js` called `cdifInterface.disconnectDevice` — neither method was ever defined on `CdifInterface`, so every POST threw `TypeError` and answered 500. `connect.js` also referenced `CHError` without importing it, so its own validation branches threw `ReferenceError` before even reaching that. Test: `test/auth/16-removed-iot-routes.js` | ➖ | ➖ | ➖ | ➖ | ➖ |
| **`/discover`, `/stop-discover`** | ➖ **removed in 7.0.0.** Mounted only under `options.allowDiscover`, which `lib/cli-options.js` hardcoded to `false` unconditionally — the comment on that line said "broken under worker thread mode". No flag existed to turn it on, so neither route was ever reachable, at any time, under any startup configuration. Test: `test/auth/16-removed-iot-routes.js` | ➖ | ➖ | ➖ | ➖ | ➖ |
| **`/devices/:deviceID/presentation`** | ➖ **removed in 7.0.0, dead twice over.** `deviceManager` never emitted the `'presentation'` event that mounts this route, so it was never reachable — and even had it been, the mount handler called `cdifInterface.getDeviceRootUrl`, a method never defined on `CdifInterface` at all, so it would have thrown `TypeError` regardless. Error codes `PRESENTATION_NOT_SUPPORTED`, `GET_DEVICE_ROOTURL_FAIL` and `PARSE_DEVICE_ROOTURL_FAIL` (both locales) are removed with it — nothing else referenced them. Test: `test/auth/16-removed-iot-routes.js` | ➖ | ➖ | ➖ | ➖ | ➖ |
| **`/load-profile`** | ➖ **removed in 7.0.0.** Existed only under `--loadProfile`, which is removed with it (unknown flags are ignored, so a startup script still passing it boots normally and gets no route). It returned `lastMinuteLoadLevel` and ignored its own interval argument; it was the only reader of the `loadLevel` counter, so the counter's initializer, its four increment sites and `getServerLoadLevel` are removed alongside it rather than left dead. Test: `test/auth/16-removed-iot-routes.js` (auth 16b) | ➖ | ➖ | ➖ | ➖ | ➖ |
| **HTTP `POST /get-module-device-list`** | ✅ `lib/routes/admin-only.js`, mounted ahead of the route (`lib/route-manager.js`). Requires `isAdmin` on the resolved session — the same device-independent capability check as the other admin routes, not device ownership: the route takes a module *name*, not a `deviceID` (`ModuleManager.prototype.getModuleDeviceListByName`, `lib/module-manager.js`). One of the two CEAMS-era package routes that survive the cleanup — it answers what a module loaded at any earlier time exposes, which `countinghouse_load_module`'s own return value cannot. Test: `test/auth/17-removed-package-routes.js`, and `test/auth/06-admin-gating.js` for the gate itself | ➖ exempt — no device action; the only input is a module name, and there is no request schema to validate against | ➖ exempt — an operator action, not a tenant call | ❌ none. Same deliberate reasoning as the module-lifecycle row above: rate-limiting an operator out of an inspection endpoint during an incident is the wrong failure mode | ➖ exempt — the route replies via `session.callbackWithoutTimer` (`lib/routes/get-module-device-list.js`), same as the module-lifecycle routes above. The lookup is a synchronous in-process read (`ModuleManager.prototype.getModuleDeviceListByName` walks `this.modules[name]` directly), not a device round-trip, so there is nothing for a timer to bound | `{topic, code, message}` — `403` from `lib/routes/admin-only.js` on an auth failure, `500` from `lib/session.js`'s `response()` if the module name is unknown. In the unknown-name case `topic`/`code` are `undefined`: `ModuleManager.prototype.getModuleDeviceListByName` rejects with a plain `Error`, not a `CHError` |
| **HTTP `GET /devices/:deviceID/package-info`** | ✅ `lib/routes/user.js` → `user-auth.js`, mounted ahead of the route on `deviceControlRouter` (`lib/route-manager.js`), same as its device-scoped neighbours `get-spec`/`schema`/`add-job`. The other CEAMS-era route that survives — it reports a loaded device's package name and version. Test: `test/auth/17-removed-package-routes.js` | ➖ exempt — no device action; it returns package metadata, not the result of a schema-validated call | ➖ exempt — it reports a loaded device's package name/version, not a billable action | ❌ **none.** The 2026-08-11 read-path rate-limit work (`test/auth/15-read-path-rate-limits.js`) covered `/balance` and the `tasks/*` and job routes; `lib/route-manager.js:89` mounts this route with no `rateLimitGate` in front of it, so it was not covered and still has no limit | ➖ **exempt — despite the family resemblance to `get-spec`/`schema`, this one has no timer.** The route replies via `session.callbackWithoutTimer` (`lib/routes/get-device-package-info.js`), and `DeviceManager.prototype.onGetDevicePackageInfo` (`lib/device-manager.js`) is a synchronous in-process read of `cdifDevice.packageInfo` that never calls `session.setDeviceTimer`. That is unlike `onGetDeviceSpec`/`onGetDeviceSchema`, which do call `setDeviceTimer` because they may round-trip to a worker-thread device; `package-info` never round-trips, so `options.requestTimeout` never starts running on this path | `{topic, code, message}` via `lib/session.js`'s `response()` — `500` for `unknown device` or `null package info`, both plain `Error`s rather than `CHError`s, so `topic`/`code` are `undefined` here the same way as the row above |
...
## Decision: which layer owns the read-path rate limit (7.0.0)

**Decision: the entry path, through one shared helper
(`lib/rate-limit-gate.js`) — not `lib/job-control.js`.**

The obvious precedent pointed the other way. Per-job *ownership* was
deliberately put down in `job-control.js` rather than at either front door,
because gating only the MCP gateway would have left the HTTP job routes as a
bypass (audit findings S1/S5). Copying that shape here would have been
wrong, for two concrete reasons:

- **It would miscount.** `handleTasksCancel` calls `JobControl.getJob` and
  then `JobControl.removeJob` — the second re-checks ownership rather than
  trusting the first, which is correct for ownership. A limiter in the same
  place would spend two of a caller's tokens on one `tasks/cancel`.
- **It could not cover the paths that needed it most.** Neither
  `GET /balance` nor `countinghouse_check_balance` touches `job-control` at
  all.

Ownership is a property of a *job*, so it belongs with the job. A rate limit
is a property of one inbound *request*, so it belongs where requests arrive.
The anti-bypass property that motivated S1/S5 is preserved differently here:
every entry path calls the one helper, and the two MCP surfaces that resolve
identity do it through a single `resolveGatedIdentity` that cannot resolve
without also checking.

**Reads share the `--apiKeyRateLimit` budget** rather than getting a second
flag. One caller gets one request budget; the cost is that a client polling
`tasks/get` draws down its own tool-call allowance. A separate
`--apiKeyReadRateLimit` was considered and rejected as public surface a
major release does not need — revisit if a real client turns out to starve
itself.

**Still fails open**, on every failure mode: flag unset, no metering
provider, unresolved key, or Redis down. Unchanged from every pre-existing
limiter, and consistent with this codebase's rule of not denying access as a
side effect of its own infrastructure being unavailable.

**What this does not cover.** The two direct-peer-channel rows remain
unlimited, per the decision above — this change did not reopen it. `tools/list`
is also still unlimited, and now has the distinction of being the last
authenticated tenant-facing path with no limit; it was left out of 7.0.0's
scope deliberately rather than by omission.
