...
| **OpenStack simulation routes** | ➖ **exempt, deliberately and by design** — mounted ahead of `lib/routes/user.js` in `lib/route-manager.js`, and the code says so at the mount point ("openstack api simulation don't do user auth"). They exist only under `--simOpenStackAPI`. (`/devices/:deviceID/wss` was listed here too until 5.0.0 removed it with the rest of the event subsystem — see the event-channel row; `/callbacks` was listed here until 7.0.0 removed it — see the row above.) Listed here so the exemption is a recorded decision rather than an omission | ⚠️ not traced | ❌ none | ❌ none | varies | varies |
